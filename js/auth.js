...
function aplicarPermisosUI() {
    ...
    if (esCentral) {
        document.querySelectorAll('[onclick^="abrirModalHistorico"], [onclick^="abrirModalArmamento"], [onclick^="abrirModalRadios"], [onclick^="toggleDesgloseEnCampo"], [onclick^="toggleDesgloseRastrillo"]').forEach(el => {
            el.style.display = 'none';
        });
        const resumenArmamento = document.getElementById('armas-global')?.closest('.bg-white');
        if (resumenArmamento) resumenArmamento.style.display = 'none';
        const totalArmas = document.getElementById('total-armas')?.closest('.bg-white\\/5');
        if (totalArmas) totalArmas.style.display = 'none';
