...
async function registrarActaServidor(d){if(!idSolicitudActaActual)idSolicitudActaActual=nuevoIdSolicitudActa();return await postActas({accion:'crear_acta_armamento',token:tokenSesionActual(),idSolicitud:idSolicitudActaActual,acta:d});}
...
async function generarActaArmamento(){if(actaGenerando)return;const d=leerFormularioActa(),err=validarDatosActa(d);if(err)return mostrarErrorActa(err);mostrarErrorActa('');const b=document.getElementById('acta-btn-generar');actaGenerando=true;b.disabled=true;try{progresoActa('Registrando datos del acta…',20);const reg=await registrarActaServidor(d);if(!reg.ok)throw new Error(reg.mensaje||'No se pudo registrar el acta');d.codigoActa=reg.codigo;progresoActa(reg.reutilizada?'La solicitud ya estaba registrada. Recuperando el acta…':'Registro guardado. Generando PDF…',45);await descargarPdfActa(d);progresoActa('Finalizando…',100);if(typeof cerrarModalArmamento==='function')cerrarModalArmamento();document.getElementById('actas-modal').style.display='none';}catch(e){mostrarErrorActa(e.message||String(e));}finally{setTimeout(cerrarProgresoActa,350);actaGenerando=false;b.disabled=false;b.textContent='📄 Registrar y generar PDF';}}
