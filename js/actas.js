...
function generarPDFGuardia(d,evidencias){
 ...
 doc.autoTable({startY:y,margin:{left:14,right:14},theme:'grid',styles:{fontSize:7.8,cellPadding:2.7},body:[['FECHA',fechaLargaEspanol(d.fecha),'CANTIDAD',armas.length],['NOMBRE',d.receptorNombre,'CÉDULA',d.receptorCedula],['CARGO',d.cargo,'ÁREA / PROYECTO',d.proyecto||'—']]});y=doc.lastAutoTable.finalY+6;
 ...
 const sy=Math.min(y+3,184),sw=96,sg=12,sx=(W-(sw*2+sg))/2;dibujarFirmaGuardia(doc,sx,sy,sw,'ENTREGA','SUPERVISOR',d.supervisorNombre||'—',d.supervisorCedula||'—');dibujarFirmaGuardia(doc,sx+sw+sg,sy,sw,'RECIBE',d.cargo,d.receptorNombre,d.receptorCedula);doc.save(`${d.codigoActa}_GUARDIA.pdf`);
}
function generarPDFCustodio(d,evidencias){
