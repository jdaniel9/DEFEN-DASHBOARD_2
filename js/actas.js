...
async function registrarActaServidor(d,confirmarInvalidacion=false){if(!idSolicitudActaActual)idSolicitudActaActual=nuevoIdSolicitudActa();return await postActas({accion:'crear_acta_armamento',token:tokenSesionActual(),idSolicitud:idSolicitudActaActual,confirmarInvalidacion,acta:d});}
    ...
    try{
        ...
        c.innerHTML=['GUARDIA','CUSTODIO'].map(t=>{
            const tarjetas=g[t].map(a=>{
                ...
                return `<div style="background:white;border:1px solid ${invalidada?'#fecaca':'#e2e8f0'};border-radius:10px;padding:10px 12px;margin-bottom:7px;display:flex;gap:10px;align-items:center;flex-wrap:wrap"><div style="flex:1;min-width:220px"><b style="font-size:12px;color:#0f172a">${escHtml(a.codigo)}</b>${estado}<div style="font-size:10px;color:#64748b">${escHtml(a.receptor)} · ${a.armas.length} arma(s) · ${escHtml(a.fecha)}</div><div style="font-size:9px;color:#94a3b8">Series: ${a.armas.map(escHtml).join(', ')}</div>${detalleInvalidacion}</div><button onclick="descargarPdfDesdeHistorial('${escAttr(a.codigo)}')" style="border:0;border-radius:7px;background:#dbeafe;color:#075985;padding:6px 8px;font-size:10px;font-weight:900;cursor:pointer">📄 Generar PDF</button>${r.esAdmin?`<button onclick="eliminarUltimaActa('${escAttr(a.codigo)}')" style="border:0;border-radius:7px;background:#fee2e2;color:#b91c1c;padding:6px 8px;font-size:10px;font-weight:900;cursor:pointer">Eliminar</button>`:''}</div>`;
            }).join('');
...
function abrirGeneradorActa(serie){if(typeof usuarioPuedeGenerarActas==='function'&&!usuarioPuedeGenerarActas())return alert('Solo Operaciones y Administrador pueden generar actas de armamento.');if(!tokenSesionActual())return alert('Tu sesión venció. Ingresa nuevamente.');asegurarModalActas();actasV3PrepararCampos();actasV3PrepararValidacionesCampos();document.getElementById('acta-tipo').value='guardia';limpiarFormularioActa();document.getElementById('acta-fecha').value=fechaISOHoy();actasV3ActualizarTipo();document.getElementById('actas-modal').style.display='flex';if(serie){const a=armasDisponiblesActa().find(x=>String(x.serie||'').trim()===String(serie).trim());if(a)actasV3ElegirArma(0,a);else mostrarErrorActa('Esta arma no puede utilizarse en un acta porque está En Tránsito, Perdida/Robada, Confiscada o no cumple las reglas del tipo seleccionado.');}}
...
async function generarActaArmamento(){
    ...
    try{
        ...
        let reg=await registrarActaServidor(d,false);
        if(reg.requiereConfirmacion){
            ...
            reg=await registrarActaServidor(d,true);
        }
