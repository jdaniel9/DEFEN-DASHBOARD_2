...
function cambiarProyectoRegularizacion(){
    ...
    const [provincia,proyecto]=clave.split('||');personalRegularizacionActual=(typeof personalBaseActas==='function'?personalBaseActas():(personalActas||[])).filter(p=>/^\d{10}$/.test(String(p.cedula||''))).sort((a,b)=>{const ar=normalizarTexto(a.provincia)===normalizarTexto(provincia)&&normalizarTexto(a.proyecto)===normalizarTexto(proyecto)?0:1,br=normalizarTexto(b.provincia)===normalizarTexto(provincia)&&normalizarTexto(b.proyecto)===normalizarTexto(proyecto)?0:1;return ar-br||String(a.nombre||'').localeCompare(String(b.nombre||''));});
    const resp=document.getElementById('regularizacion-responsable');if(resp)resp.innerHTML='<option value="">Selecciona supervisor</option>'+personalRegularizacionActual.map((p,i)=>`<option value="${i}">${escHtml(p.nombre)} · ${escHtml(p.cedula)}</option>`).join('')+'<option value="MANUAL">Escribir supervisor manualmente</option>';renderArmasRegularizacion();
}
