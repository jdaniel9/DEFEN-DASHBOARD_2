...
function armasDisponiblesRetorno(){return (armamentoDetalle||[]).filter(a=>['activo','en campo'].includes(normalizarTexto(a.estado))&&a.provincia&&a.proyecto&&a.serie);}
function armasPendientesRegularizacion(){return (armamentoDetalle||[]).filter(a=>['activo','en campo'].includes(normalizarTexto(a.estado))&&!String(a.actaVigente||'').trim()&&a.provincia&&a.proyecto&&a.serie);}
...
function nombresSupervisoresRegularizacion(provincia,proyecto){
    const detalle=detalleProvincias?.[String(provincia||'').toUpperCase()]||{},proyectoEncontrado=(detalle.proyectosList||[]).find(p=>normalizarTexto(p.nombre)===normalizarTexto(proyecto));
    return (proyectoEncontrado?.supervisores?.length?proyectoEncontrado.supervisores:(detalle.supervisores||[])).map(normalizarTexto).filter(Boolean);
}
function cambiarProyectoRegularizacion(){
    ...
    const [provincia,proyecto]=clave.split('||'),nombresConfigurados=nombresSupervisoresRegularizacion(provincia,proyecto),personal=(typeof personalBaseActas==='function'?personalBaseActas():(personalActas||[]));
    personalRegularizacionActual=personal.filter(p=>{const cargo=normalizarTexto(p.cargo),nombre=normalizarTexto(p.nombre),ubicacionCorrecta=normalizarTexto(p.provincia)===normalizarTexto(provincia)&&normalizarTexto(p.proyecto)===normalizarTexto(proyecto);return /^\d{10}$/.test(String(p.cedula||''))&&(nombresConfigurados.includes(nombre)||(ubicacionCorrecta&&/(supervisor|coordinador|jefe)/.test(cargo)));}).filter((p,i,lista)=>lista.findIndex(x=>String(x.cedula)===String(p.cedula))===i).sort((a,b)=>{const ai=nombresConfigurados.indexOf(normalizarTexto(a.nombre)),bi=nombresConfigurados.indexOf(normalizarTexto(b.nombre));return (ai<0?999:ai)-(bi<0?999:bi)||String(a.nombre||'').localeCompare(String(b.nombre||''));});
...
async function ejecutarRegularizacion(){
    ...
    if(!confirm(`¿Regularizar ${registros.length} arma(s) de ${proyecto}?\n\nPermanecerán en estado Activo y se crearán ${modo==='LOTE'?'una acta':'actas individuales'}.`))return;
...
async function cargarMovimientosTransito(){
    ...
    try{
        ...
        c.innerHTML=`<div style="background:#eff6ff;border:1px solid #93c5fd;color:#1e40af;border-radius:12px;padding:12px;margin-bottom:12px;font-size:10px;font-weight:700">Confirma la llegada únicamente cuando todas las armas del lote estén físicamente en su destino.</div>`+movimientos.map(m=>`<div style="background:white;border:1px solid #bfdbfe;border-left:4px solid ${m.tipoMovimiento==='RETORNO'?'#f97316':'#2563eb'};border-radius:10px;padding:11px 12px;margin-bottom:8px;display:flex;gap:10px;align-items:center;flex-wrap:wrap"><div style="flex:1;min-width:250px"><b style="font-size:12px;color:#0f172a">${escHtml(m.loteId||m.codigoActa)}</b><span style="display:inline-block;margin-left:7px;padding:3px 7px;border-radius:99px;background:#dbeafe;color:#1d4ed8;font-size:8px;font-weight:900">${m.tipoMovimiento==='RETORNO'?'RETORNO · ':''}EN TRÁNSITO</span>${m.estadoDocumental==='PENDIENTE_SUBSANAR'?'<span style="display:inline-block;margin-left:5px;padding:3px 7px;border-radius:99px;background:#ffedd5;color:#c2410c;font-size:8px;font-weight:900">GUÍA PENDIENTE</span>':''}<div style="font-size:10px;color:#475569;margin-top:4px">Destino: ${escHtml(m.provinciaDestino)} · ${escHtml(m.ciudadDestino)}${m.proyectoDestino?' · '+escHtml(m.proyectoDestino):''}${m.puestoDestino?' · '+escHtml(m.puestoDestino):''}</div><div style="font-size:9px;color:#94a3b8">${m.responsable?'Responsable: '+escHtml(m.responsable)+' · ':''}Series: ${(m.armas||[]).map(a=>escHtml(a.serie)).join(', ')}</div></div>${m.urlGuia?`<a href="${escAttr(m.urlGuia)}" target="_blank" rel="noopener" style="border-radius:7px;background:#ecfdf5;color:#047857;padding:7px 9px;font-size:10px;font-weight:900;text-decoration:none">Ver guía</a>`:''}<button onclick="confirmarLlegadaMovimiento('${escAttr(m.loteId||'')}','${escAttr(m.codigoActa||'')}',${Number((m.armas||[]).length)},'${escAttr(m.tipoMovimiento||'ASIGNACION')}')" style="border:0;border-radius:7px;background:${m.tipoMovimiento==='RETORNO'?'#f97316':'#2563eb'};color:white;padding:7px 10px;font-size:10px;font-weight:900;cursor:pointer">✓ Confirmar llegada</button></div>`).join('');
...
async function confirmarLlegadaMovimiento(loteId,codigoActa,cantidad,tipoMovimiento){
    const retorno=tipoMovimiento==='RETORNO',resultado=retorno?'Rastrillo y sus actas quedarán FINALIZADAS':'Activo en el proyecto y puesto de destino';
