...
async function ejecutarRegularizacionSupabase(datos){
    ...
    for(const grupo of grupos){
        ...
        const todasConGuia=armas.every(a=>Boolean(a.urlGuiaEnvio)),guias=[...new Set(armas.map(a=>a.urlGuiaEnvio).filter(Boolean))];
        const requestId=datos.modo==='LOTE'?(idSolicitudRegularizacionActual||nuevoIdSolicitudActa()):nuevoIdSolicitudActa();
        const payload={request_id:requestId,request_fingerprint:`REGULARIZACION-${requestId}`,act_type:tipo,act_date:datos.fecha,effective_at:`${datos.fechaTraslado}T12:00:00-05:00`,regularization:true,document_status:todasConGuia?'RESPALDO_EXISTENTE':'NO_APLICA',guide_url:todasConGuia&&guias.length===1?guias[0]:null,receiver:{personnel_id:primero.responsablePersonalId||null,name:primero.responsableNombre,national_id:String(primero.responsableCedula),origin:'REGULARIZACION',position_title:primero.cargo||'SUPERVISOR'},destination:{province_id:armaBase.provinciaId,project_id:armaBase.proyectoId,post_id:null,city:datos.ciudad,location:null},supervisor:tipo==='GUARDIA'?{name:datos.entregaNombre,national_id:String(datos.entregaCedula),personnel_id:null}:{},permit_reference:'N/A',comment:'REGULARIZACION DE ARMAMENTO EXISTENTE',novelty:'SIN TRASLADO ACTUAL',movement_observation:`REGULARIZACION · FECHA REAL DE TRASLADO ${datos.fechaTraslado}`,items:grupo.map((r,i)=>({weapon_id:armas[i].idArma,magazines:r.alimentadoras,ammunition:r.municiones}))};
        const respuesta=await supabaseCrearActaArmamento(payload);codigos.push(respuesta.act_code);
    }
    return{ok:true,codigos,mensaje:`Regularización completada. Se crearon ${codigos.length} acta(s) para ${datos.registros.length} arma(s).`};
}
