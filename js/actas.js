...
async function postActas(payload){const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),45000);try{const r=await fetch(APPS_SCRIPT_URL,{method:'POST',body:JSON.stringify(payload),redirect:'follow',signal:controller.signal});if(!r.ok)throw new Error(`El servidor respondió HTTP ${r.status}.`);return await r.json();}catch(e){if(e.name==='AbortError')throw new Error('El guardado en Drive tardó demasiado. El PDF local ya fue descargado y el acta quedó registrada; el historial lo mostrará como PDF pendiente.');throw e;}finally{clearTimeout(timeout);}}
async function guardarPdfHistorico(codigo,doc){const data=doc.output('datauristring');const base64=data.split(',')[1]||'';console.info(`Archivando PDF ${codigo}: ${(base64.length/1024/1024).toFixed(2)} MB codificados.`);return postActas({accion:'guardar_pdf_acta',token:tokenSesionActual(),codigo,base64});}
async function generarActaArmamento(){if(actaGenerando)return;const d=leerFormularioActa(),err=validarDatosActa(d);if(err)return mostrarErrorActa(err);mostrarErrorActa('');const b=document.getElementById('acta-btn-generar');actaGenerando=true;b.disabled=true;try{progresoActa('Registrando datos del acta…',15);const reg=await registrarActaServidor(d);if(!reg.ok)throw new Error(reg.mensaje||'No se pudo registrar el acta');d.codigoActa=reg.codigo;progresoActa('Recuperando evidencias fotográficas…',40);const ev=await Promise.all(d.armas.map(async a=>({cred:await imagenActaBase64(a.urlCredencial),arma:await imagenActaBase64(a.urlArma)})));progresoActa('Construyendo el PDF…',70);const doc=d.tipoActa==='CUSTODIO VIP'?generarPDFCustodio(d,ev):generarPDFGuardia(d,ev);progresoActa('Descargando PDF…',82);doc.save(`${d.codigoActa}_${d.tipoActa==='CUSTODIO VIP'?'CUSTODIO':'GUARDIA'}.pdf`);progresoActa('Guardando copia en Drive…',90);try{const pdf=await guardarPdfHistorico(d.codigoActa,doc);if(!pdf.ok)throw new Error(pdf.mensaje||'No se pudo guardar el PDF en Drive.');}catch(archivoError){console.warn('PDF pendiente de archivar:',archivoError);alert(`El acta ${d.codigoActa} fue registrada y el PDF se descargó. No se pudo guardar su copia en Drive: ${archivoError.message}`);}progresoActa('Finalizando…',100);if(typeof cerrarModalArmamento==='function')cerrarModalArmamento();document.getElementById('actas-modal').style.display='none';}catch(e){mostrarErrorActa(e.message||String(e));}finally{setTimeout(cerrarProgresoActa,350);actaGenerando=false;b.disabled=false;b.textContent='📄 Registrar y generar PDF';}}
