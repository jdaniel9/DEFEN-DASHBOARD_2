...
function actasV3PrepararPestanaPendientes(){
    if(document.getElementById('acta-tab-pendientes'))return;
    ...
    boton.id='acta-tab-pendientes';boton.textContent='⚠️ Pendientes de subsanar';boton.onclick=()=>mostrarPestanaActas('pendientes');
    boton.style.cssText='border:0;border-radius:8px;padding:7px 12px;background:#e2e8f0;color:#334155;font-size:11px;font-weight:900;cursor:pointer';
    tabHistorial.after(boton);
    const vista=document.createElement('div');vista.id='acta-vista-pendientes';vista.style.cssText='display:none;overflow:auto;padding:16px 18px';vista.innerHTML='<div id="actas-pendientes-lista"></div>';
    vistaHistorial.after(vista);
}
function mostrarPestanaActas(vista){
    const historial=vista==='historial',pendientes=vista==='pendientes',nueva=!historial&&!pendientes;
    ...
    [['acta-tab-nueva',nueva],['acta-tab-historial',historial],['acta-tab-pendientes',pendientes]].forEach(([id,activo])=>{const e=document.getElementById(id);if(e)e.style.cssText=`border:0;border-radius:8px;padding:7px 12px;font-size:11px;font-weight:900;cursor:pointer;${activo?'background:#f97316;color:white':'background:#e2e8f0;color:#334155'}`;});
