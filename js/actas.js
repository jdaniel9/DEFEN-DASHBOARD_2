...
function actasV3ProvinciaId(nombre){const proyectos=Array.isArray(supabaseWeaponWorkspace?.projects)?supabaseWeaponWorkspace.projects:[];return proyectos.find(p=>normalizarTexto(p.province)===normalizarTexto(nombre))?.province_id||null;}
async function resolverDestinoActaSupabase(d){
    const proyectos=Array.isArray(supabaseWeaponWorkspace?.projects)?supabaseWeaponWorkspace.projects:[];
    const existente=proyectos.find(p=>normalizarTexto(p.province)===normalizarTexto(d.provincia)&&normalizarTexto(p.name)===normalizarTexto(d.proyecto));
    const provinceId=existente?.province_id||actasV3ProvinciaId(d.provincia);
    if(!provinceId)throw new Error('La provincia seleccionada no fue reconocida en Supabase.');
    let projectId=existente?.id||null,postId=null;
    if(d.crearProyecto){
        try{const creado=await supabaseGuardarProyecto({request_id:nuevoIdSolicitudActa(),province_id:provinceId,name:d.proyecto,contract_type:d.tipoContrato,start_date:d.fechaProyectoInicio,planned_end_date:d.fechaProyectoFin});projectId=creado?.project_id||null;}
        catch(error){if(!/ya existe un proyecto/i.test(error?.message||''))throw error;await cargarWorkspaceArmamentoSupabase(true);projectId=supabaseWeaponWorkspace.projects.find(p=>normalizarTexto(p.province)===normalizarTexto(d.provincia)&&normalizarTexto(p.name)===normalizarTexto(d.proyecto))?.id||null;}
    }
    if(!projectId)throw new Error('El proyecto seleccionado no está activo o no fue reconocido en Supabase.');
    const puestoExistente=(Array.isArray(existente?.posts)?existente.posts:[]).find(p=>normalizarTexto(p.name)===normalizarTexto(d.puesto));
    postId=puestoExistente?.id||null;
    if(d.crearPuesto){
        try{const creado=await supabaseGuardarPuestoProyecto({request_id:nuevoIdSolicitudActa(),project_id:projectId,name:d.puesto,service_type:d.tipoServicioPuesto||null,shift_label:d.turnoPuesto||null,service_days:d.diasServicioPuesto||null,armed:true,has_radio:Boolean(d.puestoRadio),active:true,observation:'CREADO DESDE ACTA DE ARMAMENTO'});postId=creado?.post_id||null;}
        catch(error){if(!/ya existe un puesto/i.test(error?.message||''))throw error;await cargarWorkspaceArmamentoSupabase(true);const pr=supabaseWeaponWorkspace.projects.find(p=>p.id===projectId);postId=(pr?.posts||[]).find(p=>normalizarTexto(p.name)===normalizarTexto(d.puesto))?.id||null;}
    }
    if(!postId)throw new Error('Selecciona un puesto existente o crea uno nuevo con la opción OTRO.');
    return{provinceId,projectId,postId};
}
async function registrarActaServidor(d,confirmarInvalidacion=false,guia=null){
    ...
    const destino=await resolverDestinoActaSupabase(d),supervisor=supervisoresDestino().find(s=>normalizarTexto(s.nombre)===normalizarTexto(d.supervisorNombre));
    ...
    const payload={
        ...
        destination:{province_id:destino.provinceId,project_id:destino.projectId,post_id:destino.postId,city:d.ciudad,location:d.ubicacionNominal||d.puesto||null},
    ...
    return{ok:Boolean(r?.ok),codigo:r?.act_code||'',loteId:r?.batch_id||'',pendienteSubsanar:r?.document_status==='PENDIENTE',pendienteRecepcion:r?.document_status==='PENDIENTE_RECEPCION',reutilizada:Boolean(r?.idempotent),resultado:r};
}
async function imagenActaBase64(url,rutaPrivada=''){
    if(backendUsaSupabase()&&rutaPrivada){
        try{return await supabaseEvidenciaArmamentoBase64(rutaPrivada);}
        catch(e){console.warn('Evidencia privada del acta:',e);return '';}
    }
    if(!url)return '';
    try{const j=await postActas({accion:'imagen_acta',token:tokenSesionActual(),url});if(j.ok&&j.base64)return `data:${j.mime||'image/jpeg'};base64,${j.base64}`;}
    catch(e){console.warn('Imagen acta:',e);}
    return '';
}
...
function actasPestanaDisponible(vista){
    ...
    if(vista==='retorno')return typeof SUPABASE_WEAPON_RETURN_ENABLED!=='undefined'&&SUPABASE_WEAPON_RETURN_ENABLED;
...
function cambiarProyectoRegularizacion(){
    ...
    const [provincia,proyecto]=clave.split('||');personalRegularizacionActual=supervisoresConfiguradosProyecto(provincia,proyecto);
    const resp=document.getElementById('regularizacion-responsable'),manual=document.getElementById('regularizacion-responsable-manual'),cedula=document.getElementById('regularizacion-responsable-cedula');if(resp)resp.innerHTML='<option value="">'+(personalRegularizacionActual.length?'Selecciona supervisor':'No hay supervisores registrados')+'</option>'+personalRegularizacionActual.map((p,i)=>`<option value="${i}">${escHtml(p.nombre)} · ${escHtml(p.cedula||'CÉDULA PENDIENTE')}</option>`).join('')+'<option value="MANUAL">Escribir supervisor manualmente</option>';if(manual){manual.value='';manual.style.display='none';}if(cedula)cedula.value='';renderArmasRegularizacion();
}
function opcionesPersonalRegularizacion(cedula=''){return '<option value="">Selecciona responsable</option>'+personalRegularizacionActual.map((p,i)=>`<option value="${i}" ${String(p.cedula)===String(cedula)?'selected':''}>${escHtml(p.nombre)} · ${escHtml(p.cedula||'CÉDULA PENDIENTE')}</option>`).join('');}
...
function actualizarResponsableRegularizacion(serie,indice){const p=indice===''?null:personalRegularizacionActual[Number(indice)];if(!configuracionRegularizacionIndividual[serie])configuracionRegularizacionIndividual[serie]={};configuracionRegularizacionIndividual[serie].responsablePersonalId=p?.personalId||null;configuracionRegularizacionIndividual[serie].responsableNombre=p?.nombre||'';configuracionRegularizacionIndividual[serie].responsableCedula=p?.cedula||'';}
async function ejecutarRegularizacion(){
    ...
    if(modo==='LOTE'){const tipoActa=document.getElementById('regularizacion-tipo').value,valorResponsable=document.getElementById('regularizacion-responsable').value,p=valorResponsable===''||valorResponsable==='MANUAL'?null:personalRegularizacionActual[Number(valorResponsable)],nombreResponsable=valorResponsable==='MANUAL'?(document.getElementById('regularizacion-responsable-manual')?.value.trim()||''):(p?.nombre||''),cedula=document.getElementById('regularizacion-responsable-cedula').value.trim();if(!nombreResponsable||!/^\d{10}$/.test(cedula))return alert('Selecciona o escribe el supervisor responsable y verifica su cédula.');if(tipoActa==='GUARDIA'&&!entregaNombre)return alert('Indica quién entrega para el acta de Guardia.');if(tipoActa==='CUSTODIO VIP'&&seleccionadas.some(a=>!normalizarTexto(a.categoria).includes('movil')))return alert('Para Custodio/VIP todas las armas seleccionadas deben ser categoría Móvil.');registros=seleccionadas.map(a=>({serie:a.serie,tipoActa,responsablePersonalId:p?.personalId||null,responsableNombre:nombreResponsable,responsableCedula:cedula,cargo:'SUPERVISOR',puesto:a.puesto||'',alimentadoras,municiones:alimentadoras*(tipoActa==='CUSTODIO VIP'?10:5),urlCredencial:a.urlCredencial||'',urlArma:a.urlImagenArma||''}));}
    else{for(const a of seleccionadas){const cfg=configuracionRegularizacionIndividual[a.serie]||{};if(!cfg.responsableNombre)return alert(`Selecciona un responsable para la serie ${a.serie}.`);if(!/^\d{10}$/.test(String(cfg.responsableCedula||''))){const cedulaIngresada=String(prompt(`El supervisor ${cfg.responsableNombre} no tiene cédula registrada.\n\nIngresa su cédula de 10 números para la serie ${a.serie}:`)||'').replace(/\D/g,'');if(!/^\d{10}$/.test(cedulaIngresada))return alert(`La cédula del responsable de la serie ${a.serie} debe contener exactamente 10 números.`);cfg.responsableCedula=cedulaIngresada;}if(cfg.tipoActa==='GUARDIA'&&!entregaNombre)return alert('Indica quién entrega para las actas de Guardia.');const tipoActa=cfg.tipoActa||'GUARDIA';registros.push({serie:a.serie,tipoActa,responsablePersonalId:cfg.responsablePersonalId||null,responsableNombre:cfg.responsableNombre,responsableCedula:cfg.responsableCedula,cargo:'RESPONSABLE',puesto:a.puesto||'',alimentadoras,municiones:alimentadoras*(tipoActa==='CUSTODIO VIP'?10:5),urlCredencial:a.urlCredencial||'',urlArma:a.urlImagenArma||''});}}
    if(registros.some(r=>r.tipoActa==='GUARDIA')&&!/^\d{10}$/.test(entregaCedula))return alert('Para las actas de Guardia, la cédula de quien entrega debe contener exactamente 10 números.');
    const archivoGuia=document.getElementById('regularizacion-guia-pdf')?.files?.[0]||null;if(archivoGuia&&(!archivoGuia.name.toLowerCase().endsWith('.pdf')||(archivoGuia.type&&archivoGuia.type!=='application/pdf')))return alert('La guía histórica debe estar en formato PDF.');if(archivoGuia&&archivoGuia.size>10*1024*1024)return alert('La guía histórica no puede superar 10 MB.');
    if(!confirm(`¿Regularizar ${registros.length} arma(s) de ${proyecto}?\n\nEl tiempo fuera del rastrillo se contará desde ${fechaTraslado}. Permanecerán en estado Activo y se crearán ${modo==='LOTE'?'una acta':'actas individuales'}.${archivoGuia?' La guía PDF quedará asociada al historial.':' El documento quedará pendiente de subsanar.'}`))return;
    try{btn.disabled=true;progresoActa('Registrando regularización…',25);const r=backendUsaSupabase()?await ejecutarRegularizacionSupabase({modo,fecha,fechaTraslado,ciudad,provincia,proyecto,entregaNombre,entregaCedula,registros,archivoGuia}):await postActas({accion:'regularizar_armas',token:tokenSesionActual(),idSolicitud:idSolicitudRegularizacionActual||nuevoIdSolicitudActa(),modo,fecha,fechaTraslado,ciudad,provincia,proyecto,entregaNombre,entregaCedula,registros},90000);if(!r.ok)throw new Error(r.mensaje);idSolicitudRegularizacionActual='';const entradaGuia=document.getElementById('regularizacion-guia-pdf');if(entradaGuia)entradaGuia.value='';progresoActa('Generando acta(s)…',55);for(const codigo of (r.codigos||[])){const detalle=await postActas({accion:'obtener_acta',token:tokenSesionActual(),codigo});if(detalle.ok)await descargarPdfActa(detalle.acta);}alert(r.mensaje);if(typeof cargarDatos==='function')await cargarDatos();if(backendUsaSupabase()){invalidarWorkspaceArmamentoSupabase();await cargarWorkspaceArmamentoSupabase(true);if(typeof renderTablaArmamento==='function')renderTablaArmamento();}mostrarPestanaActas('historial');}catch(e){alert(e.message||String(e));}finally{btn.disabled=false;setTimeout(cerrarProgresoActa,350);}
}

async function ejecutarRegularizacionSupabase(datos){
    const grupos=datos.modo==='LOTE'?[datos.registros]:datos.registros.map(r=>[r]),codigos=[];let rutaGuia='';
    try{
      if(datos.archivoGuia){progresoActa('Subiendo guía histórica…',15);const carga=await supabaseSubirGuiaArmamento(datos.archivoGuia,idSolicitudRegularizacionActual||nuevoIdSolicitudActa(),'regularization');rutaGuia=carga.path;}
      for(const grupo of grupos){
        const armas=grupo.map(r=>armamentoDetalle.find(a=>String(a.serie)===String(r.serie)));
        if(armas.some(a=>!a?.idArma||!a.proyectoId||!a.provinciaId))throw new Error('Una de las armas no tiene identificadores válidos en Supabase.');
        const primero=grupo[0],armaBase=armas[0],tipo=primero.tipoActa==='CUSTODIO VIP'?'CUSTODIO':'GUARDIA';
        if(armas.some(a=>a.proyectoId!==armaBase.proyectoId||a.provinciaId!==armaBase.provinciaId))throw new Error('Todas las armas del lote deben pertenecer al mismo proyecto.');
        if(tipo==='GUARDIA'&&!/^\d{10}$/.test(String(datos.entregaCedula||'')))throw new Error('Para Guardia, la cédula de quien entrega debe contener 10 números.');
        const todasConGuia=armas.every(a=>actasUrlGuiaHistoricaValida(a.urlGuiaEnvio)),guias=[...new Set(armas.map(a=>a.urlGuiaEnvio).filter(actasUrlGuiaHistoricaValida))];
        const requestId=datos.modo==='LOTE'?(idSolicitudRegularizacionActual||nuevoIdSolicitudActa()):nuevoIdSolicitudActa();
        const payload={request_id:requestId,request_fingerprint:`REGULARIZACION-${requestId}`,act_type:tipo,act_date:datos.fecha,effective_at:`${datos.fechaTraslado}T12:00:00-05:00`,regularization:true,document_status:rutaGuia?'COMPLETO':todasConGuia?'RESPALDO_EXISTENTE':'PENDIENTE',guide_storage_path:rutaGuia||null,guide_url:!rutaGuia&&todasConGuia&&guias.length===1?guias[0]:null,receiver:{personnel_id:primero.responsablePersonalId||null,name:primero.responsableNombre,national_id:String(primero.responsableCedula),origin:'REGULARIZACION',position_title:primero.cargo||'SUPERVISOR'},destination:{province_id:armaBase.provinciaId,project_id:armaBase.proyectoId,post_id:null,city:datos.ciudad,location:null},supervisor:tipo==='GUARDIA'?{name:datos.entregaNombre,national_id:String(datos.entregaCedula),personnel_id:null}:{},permit_reference:'N/A',comment:'REGULARIZACION DE ARMAMENTO EXISTENTE',novelty:'SIN TRASLADO ACTUAL',movement_observation:`REGULARIZACION · FECHA REAL DE TRASLADO ${datos.fechaTraslado}`,items:grupo.map((r,i)=>({weapon_id:armas[i].idArma,magazines:r.alimentadoras,ammunition:r.municiones}))};
        const respuesta=await supabaseCrearActaArmamento(payload);codigos.push(respuesta.act_code);
      }
      return{ok:true,codigos,mensaje:`Regularización completada. Se crearon ${codigos.length} acta(s) para ${datos.registros.length} arma(s).${rutaGuia?' La guía PDF quedó vinculada al historial.':' La guía quedó pendiente de subsanar.'}`};
    }catch(e){if(rutaGuia&&!codigos.length)try{await supabaseEliminarGuiaArmamento(rutaGuia);}catch(_){}throw e;}
}
...
async function iniciarRetornoArmamento(){
    ...
    if(!archivo)return alert('La guía PDF es obligatoria para iniciar el retorno.');
    if(!archivo.name.toLowerCase().endsWith('.pdf')||(archivo.type&&archivo.type!=='application/pdf'))return alert('La guía debe estar en formato PDF.');
    if(archivo.size>10*1024*1024)return alert('La guía PDF no puede superar 10 MB.');
    ...
    try{
        btn.disabled=true;progresoActa('Subiendo guía e iniciando retorno…',35);
        let r;
        if(backendUsaSupabase()){
            const solicitud=idSolicitudRetornoActual||nuevoIdSolicitudActa(),armas=series.map(s=>armamentoDetalle.find(a=>String(a.serie)===String(s)));
            if(armas.some(a=>!a?.idArma))throw new Error('Una de las armas no tiene identificador válido en Supabase.');
            const provinciaId=armamentoDetalle.find(a=>normalizarTexto(a.provincia)===normalizarTexto(destino))?.provinciaId||supabaseWeaponWorkspace?.projects?.find(p=>normalizarTexto(p.province)===normalizarTexto(destino))?.province_id;
            if(!provinciaId)throw new Error('No se pudo identificar la provincia del rastrillo de destino.');
            const carga=await supabaseSubirGuiaArmamento(archivo,solicitud,'returns');
            r=await supabaseIniciarRetornoArmamento({request_id:solicitud,request_fingerprint:`RETORNO-${solicitud}`,weapon_ids:armas.map(a=>a.idArma),destination_province_id:provinciaId,destination_city:destino==='GUAYAS'?'GUAYAQUIL':destino==='MANABI'?'MANTA':'QUITO',destination_location:`RASTRILLO ${destino}`,effective_at:`${fecha}T12:00:00-05:00`,guide_storage_path:carga.path,observation:observacion||`RETORNO A RASTRILLO ${destino}`});
        }else{
            const guia=await actasV3ArchivoPdfABase64(archivo);r=await postActas({accion:'iniciar_retorno_armas',token:tokenSesionActual(),idSolicitud:idSolicitudRetornoActual||nuevoIdSolicitudActa(),series,destino,fecha,observacion,guia},90000);
        }
        if(!r.ok)throw new Error(r.mensaje||'No se pudo iniciar el retorno.');
        progresoActa('Retorno registrado.',100);alert(backendUsaSupabase()?`${r.weapon_count||series.length} arma(s) quedaron EN TRÁNSITO hacia ${destinoTexto}.`:r.mensaje);idSolicitudRetornoActual='';seriesRetornoSeleccionadas=new Set();if(typeof cargarDatos==='function')await cargarDatos();if(backendUsaSupabase()){invalidarWorkspaceArmamentoSupabase();invalidarWorkspaceDespachoArmamentoSupabase();await Promise.all([cargarWorkspaceArmamentoSupabase(true),cargarWorkspaceDespachoArmamentoSupabase(true)]);if(typeof renderTablaArmamento==='function')renderTablaArmamento();}mostrarPestanaActas('transito');
    ...
    try{
        ...
        c.innerHTML=['GUARDIA','CUSTODIO'].map(t=>{
            const tarjetas=g[t].map(a=>{
                const estadoNormal=normalizarTexto(a.estadoActa),invalidada=estadoNormal==='invalidada',finalizada=estadoNormal==='finalizada',parcial=estadoNormal==='vigente parcial',enTransito=estadoNormal==='en_transito';
                const colorEstado=invalidada?['#fee2e2','#b91c1c']:finalizada?['#e2e8f0','#475569']:enTransito?['#dbeafe','#1d4ed8']:parcial?['#fef3c7','#92400e']:['#dcfce7','#15803d'];
                const estado=`<span style="display:inline-block;margin-left:7px;padding:3px 7px;border-radius:99px;background:${colorEstado[0]};color:${colorEstado[1]};font-size:8px;font-weight:900">${escHtml(enTransito?'EN TRÁNSITO':a.estadoActa||'VIGENTE')}</span>`;
                const guiaPendiente=a.estadoDocumental==='PENDIENTE_SUBSANAR'||a.estadoDocumental==='PENDIENTE',guiaRecepcion=a.estadoDocumental==='PENDIENTE_RECEPCION';
                const respaldoExistente=a.estadoDocumental==='RESPALDO_EXISTENTE',tieneGuia=Boolean(a.urlGuia||a.rutaGuia),estadoGuia=`<span style="display:inline-block;margin-left:5px;padding:3px 7px;border-radius:99px;background:${guiaPendiente?'#ffedd5':guiaRecepcion?'#fef3c7':'#e0f2fe'};color:${guiaPendiente?'#c2410c':guiaRecepcion?'#92400e':'#0369a1'};font-size:8px;font-weight:900">${guiaPendiente?'GUÍA PENDIENTE':guiaRecepcion?'GUÍA CARGADA · PENDIENTE RECEPCIÓN':respaldoExistente?'RESPALDO EXISTENTE':tieneGuia?'GUÍA COMPLETA':'SIN CONTROL DE GUÍA'}</span>`;
...
function actasV3PrepararDestinoDinamico(){
 const proyectoOtro=document.getElementById('acta-proyecto-otro'),puestoOtro=document.getElementById('acta-puesto-otro');
 if(proyectoOtro&&!document.getElementById('acta-proyecto-nuevo-config')){const b=document.createElement('div');b.id='acta-proyecto-nuevo-config';b.style.cssText='display:none;margin-top:8px;padding:10px;background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px';b.innerHTML='<p class="acta-help" style="font-weight:800;color:#1d4ed8;margin:0 0 8px">El proyecto se creará al registrar el acta</p><label class="acta-label">Tipo de proyecto</label><select id="acta-proyecto-tipo" class="acta-input"><option value="">Selecciona</option><option>ORDEN DE COMPRA</option><option>CONTRATO/LICITACION</option><option>OTRO</option></select><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px"><div><label class="acta-label">Fecha de ingreso</label><input id="acta-proyecto-inicio" type="date" class="acta-input"></div><div><label class="acta-label">Fecha de finalización</label><input id="acta-proyecto-fin" type="date" class="acta-input"></div></div>';proyectoOtro.parentElement.appendChild(b);}
 if(puestoOtro&&!document.getElementById('acta-puesto-nuevo-config')){const b=document.createElement('div');b.id='acta-puesto-nuevo-config';b.style.cssText='display:none;margin-top:8px;padding:10px;background:#f8fafc;border:1px solid #cbd5e1;border-radius:10px';b.innerHTML='<p class="acta-help" style="font-weight:800;color:#334155;margin:0 0 8px">El puesto se creará y contará en el dashboard</p><label class="acta-label">Servicio</label><input id="acta-puesto-servicio" class="acta-input" value="24 HORAS" maxlength="80"><label class="acta-label" style="margin-top:7px">Turno</label><input id="acta-puesto-turno" class="acta-input" value="D/N" maxlength="40"><label class="acta-label" style="margin-top:7px">Días de servicio</label><input id="acta-puesto-dias" class="acta-input" value="LUNES A DOMINGO" maxlength="100"><label style="display:flex;gap:7px;align-items:center;margin-top:8px;font-size:11px;font-weight:800;color:#475569"><input id="acta-puesto-radio" type="checkbox"> Este puesto también requiere radio</label>';puestoOtro.parentElement.appendChild(b);}
 if(!document.getElementById('acta-ubicacion-custodio-wrap')){const receptor=document.getElementById('acta-receptor-cedula')?.parentElement;if(receptor){const b=document.createElement('div');b.id='acta-ubicacion-custodio-wrap';b.style.cssText='display:none;grid-column:1/-1;padding:10px;background:#fff7ed;border:1px solid #fed7aa;border-radius:10px';b.innerHTML='<label class="acta-label">Ubicación nominal del arma</label><input id="acta-ubicacion-custodio" class="acta-input" maxlength="180" placeholder="NOMBRE DEL CUSTODIO · CI 0000000000"><p class="acta-help">Para dotación personal se guardará como ubicación el nombre y la cédula del custodio.</p>';receptor.parentElement.appendChild(b);}}
 ['acta-receptor-nombre','acta-receptor-cedula'].forEach(id=>{const e=document.getElementById(id);if(e&&!e.dataset.ubicacionCustodio){e.addEventListener('input',actasV3ActualizarUbicacionCustodio);e.dataset.ubicacionCustodio='1';}});const inicio=document.getElementById('acta-proyecto-inicio');if(inicio&&!inicio.value)inicio.value=fechaISOHoy();actasV3ActualizarDestinoDinamico();
}
function actasV3EsGuayas(){return normalizarTexto(document.getElementById('acta-provincia-destino')?.value)==='guayas';}
function actasV3ActualizarDestinoDinamico(){const nuevoProyecto=document.getElementById('acta-proyecto-destino')?.value==='OTRO',nuevoPuesto=document.getElementById('acta-puesto-destino')?.value==='OTRO',custodio=document.getElementById('acta-tipo')?.value==='custodio';const pc=document.getElementById('acta-proyecto-nuevo-config'),puc=document.getElementById('acta-puesto-nuevo-config'),uc=document.getElementById('acta-ubicacion-custodio-wrap');if(pc)pc.style.display=nuevoProyecto?'block':'none';if(puc)puc.style.display=nuevoPuesto?'block':'none';if(uc)uc.style.display=custodio?'block':'none';const ayuda=document.getElementById('acta-guia-ayuda');if(ayuda)ayuda.innerHTML=actasV3EsGuayas()?'<b>Guayas:</b> la guía es opcional por tratarse de movilización desde la matriz.':'<b>Fuera de Guayas:</b> la guía PDF es obligatoria para Operaciones; solo Administrador puede autorizar una emergencia pendiente.';}
function actasV3ActualizarUbicacionCustodio(){const e=document.getElementById('acta-ubicacion-custodio');if(!e||document.getElementById('acta-tipo')?.value!=='custodio')return;const n=document.getElementById('acta-receptor-nombre')?.value.trim(),c=document.getElementById('acta-receptor-cedula')?.value.trim();e.value=n?`${n}${c?' · CI '+c:''}`:'';}
function actasV3PrepararGuia(){if(document.getElementById('acta-guia-pdf'))return;const novedad=document.getElementById('acta-novedad'),card=novedad?.parentElement?.parentElement;if(!card)return;const bloque=document.createElement('div');bloque.style.gridColumn='1/-1';bloque.innerHTML=`<label class="acta-label">Guía de movilización (PDF · máximo 10 MB)</label><input id="acta-guia-pdf" type="file" accept="application/pdf,.pdf" class="acta-input"><p id="acta-guia-ayuda" class="acta-help"></p>`;card.appendChild(bloque);actasV3ActualizarDestinoDinamico();}
function actasV3PrepararGuiaRegularizacion(){if(document.getElementById('regularizacion-guia-pdf'))return;const datos=document.getElementById('regularizacion-datos-lote'),card=datos?.parentElement;if(!card)return;const bloque=document.createElement('div');bloque.id='regularizacion-guia-bloque';bloque.style.marginTop='10px';bloque.innerHTML=`<label class="acta-label">Guía histórica de asignación (PDF · máximo 10 MB)</label><input id="regularizacion-guia-pdf" type="file" accept="application/pdf,.pdf" class="acta-input"><p class="acta-help">Adjunta el documento real usado para movilizar el armamento. Si todavía no lo tienes, el acta quedará en <b>Pendientes de subsanar</b>; una dirección del dashboard nunca se guardará como guía.</p>`;card.appendChild(bloque);}
function actasUrlGuiaHistoricaValida(valor){try{const u=new URL(String(valor||''),location.href),ruta=u.pathname.replace(/\/+$/,'').toLowerCase();if(!['http:','https:'].includes(u.protocol))return false;if(u.hostname.endsWith('supabase.com')&&u.pathname.includes('/dashboard/'))return false;if(u.hostname===location.hostname&&(ruta===location.pathname.replace(/\/+$/,'').toLowerCase()||ruta.endsWith('/defen-dashboard_2')||ruta.endsWith('/index.html')))return false;return true;}catch(_){return false;}}
...
function actasV3ValidarGuiaCliente(){const archivo=document.getElementById('acta-guia-pdf')?.files?.[0],excepcionActiva=typeof SUPABASE_WEAPON_GUIDE_REMEDIATION_ENABLED!=='undefined'&&SUPABASE_WEAPON_GUIDE_REMEDIATION_ENABLED;if(!archivo&&actasV3EsGuayas())return '';if(!archivo&&backendUsaSupabase()&&!(rolActual()==='admin'&&excepcionActiva))return 'La guía PDF es obligatoria cuando el armamento sale fuera de la provincia del Guayas.';if(!archivo&&rolActual()!=='admin')return 'La guía PDF es obligatoria cuando el armamento sale fuera de la provincia del Guayas.';if(archivo&&!archivo.name.toLowerCase().endsWith('.pdf'))return 'La guía debe tener extensión .pdf.';if(archivo&&archivo.size>10*1024*1024)return 'La guía PDF no puede superar 10 MB.';return '';}
...
async function abrirGeneradorActa(serie){
    ...
    try{
        ...
        asegurarModalActas();actasV3PrepararPestanaPendientes();actasV3PrepararCampos();actasV3PrepararValidacionesCampos();actasV3PrepararGuia();actasV3PrepararDestinoDinamico();
        mostrarPestanaActas('nueva');document.getElementById('acta-tipo').value='guardia';limpiarFormularioActa();actasV3LimpiarGuia();['acta-proyecto-tipo','acta-proyecto-fin','acta-ubicacion-custodio'].forEach(id=>{const e=document.getElementById(id);if(e)e.value='';});const inicioProyecto=document.getElementById('acta-proyecto-inicio');if(inicioProyecto)inicioProyecto.value=fechaISOHoy();document.getElementById('acta-fecha').value=fechaISOHoy();actasV3ActualizarTipo();document.getElementById('actas-modal').style.display='flex';
...
async function abrirRegularizacionArmamento(){
    ...
    if(!tokenSesionActual())return alert('Tu sesión venció. Ingresa nuevamente.');
    try{
        progresoActa('Cargando armamento pendiente de regularizar…',30);
        if(backendUsaSupabase())await cargarWorkspaceArmamentoSupabase();
        asegurarModalActas();actasV3PrepararPestanaPendientes();actasV3PrepararCampos();actasV3PrepararValidacionesCampos();actasV3PrepararGuia();actasV3PrepararGuiaRegularizacion();
        const cedulaEntrega=document.getElementById('regularizacion-entrega-cedula');
...
function actasV3ElegirAgente(i){const p=agentesActaFiltrados[i];if(!p)return;agenteActaSeleccionado=p;indiceAgenteActa=i;document.getElementById('acta-receptor-nombre').value=p.nombre||'';document.getElementById('acta-receptor-cedula').value=p.cedula||'';actasV3CargarUbicaciones(p.provincia,p.proyecto,p.puesto);document.getElementById('acta-agente-resultados').style.display='none';document.getElementById('acta-agente-busqueda').value=p.nombre||'';actasV3ActualizarUbicacionCustodio();cargarSupervisoresActa();}
...
function adaptarActaSupabaseDetalle(a){
    const items=Array.isArray(a?.items)?a.items:[],primero=items[0]||{},puestos=[...new Set(items.map(i=>i.post).filter(Boolean))];
    return{
        codigoActa:a?.act_code||'',tipoActa:a?.act_type==='CUSTODIO'?'CUSTODIO VIP':'GUARDIA',fecha:a?.act_date||'',ciudad:a?.city||'',
...
async function generarActaArmamento(){
    ...
    if(!tieneGuia&&!actasV3EsGuayas()&&rolActual()==='admin'&&!confirm('Esta acta se generará SIN GUÍA por autorización de Administrador.\n\nEl movimiento quedará PENDIENTE DE SUBSANAR hasta que se adjunte el PDF.\n\n¿Deseas continuar?'))return;
    ...
    let guiaStoragePath='';
    try{
        progresoActa(tieneGuia?'Preparando y cargando la guía PDF…':actasV3EsGuayas()?'Registrando movilización local en Guayas…':'Registrando emergencia sin guía…',15);
        let guia=null;
        ...
        else if(reg.pendienteRecepcion)alert(`El acta ${reg.codigo} quedó EN TRÁNSITO. Pasará a VIGENTE y la guía quedará COMPLETA cuando confirmes la llegada del lote.`);
...
async function descargarPdfActa(d){progresoActa('Recuperando evidencias fotográficas…',60);const ev=await Promise.all((d.armas||[]).map(async a=>({cred:await imagenActaBase64(a.urlCredencial,a.rutaCredencial),arma:await imagenActaBase64(a.urlArma,a.rutaArma)})));progresoActa('Construyendo PDF…',80);const doc=d.tipoActa==='CUSTODIO VIP'?generarPDFCustodio(d,ev):generarPDFGuardia(d,ev);marcarPdfActaInvalidada(doc,d);doc.save(`${d.codigoActa}_${d.tipoActa==='CUSTODIO VIP'?'CUSTODIO':'GUARDIA'}.pdf`);}
...
async function eliminarUltimaActa(codigo){
    const motivo=prompt(`Indica el motivo para eliminar ${codigo}:`,'ACTA CREADA POR ERROR');
    if(motivo===null)return;if(motivo.trim().length<8)return alert('EL MOTIVO DEBE TENER AL MENOS 8 CARACTERES.');
    if(!confirm(`¿Eliminar ${codigo}? Solo se permite si es la última operación de sus armas. Se conservará un respaldo de auditoría.`))return;
    try{
        progresoActa('Eliminando acta y restaurando el estado anterior…',35);
        const r=await postActas({accion:'eliminar_ultima_acta',token:tokenSesionActual(),codigo,motivo:motivo.trim()},90000);
