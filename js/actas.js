...
function mostrarPestanaActas(vista){
    const historial=vista==='historial',pendientes=vista==='pendientes',transito=vista==='transito',nueva=!historial&&!pendientes&&!transito;
    ...
    [['acta-tab-nueva',nueva],['acta-tab-historial',historial],['acta-tab-pendientes',pendientes],['acta-tab-transito',transito]].forEach(([id,activo])=>{const e=document.getElementById(id);if(e)e.style.cssText=`border:0;border-radius:8px;padding:7px 12px;font-size:11px;font-weight:900;cursor:pointer;${activo?'background:#f97316;color:white':'background:#e2e8f0;color:#334155'}`;});
...
async function cargarMovimientosTransito(){
    ...
    try{
        ...
        c.innerHTML=`<div style="background:#eff6ff;border:1px solid #93c5fd;color:#1e40af;border-radius:12px;padding:12px;margin-bottom:12px;font-size:10px;font-weight:700">Confirma la llegada únicamente cuando todas las armas del acta estén físicamente en el proyecto o puesto indicado.</div>`+movimientos.map(m=>`<div style="background:white;border:1px solid #bfdbfe;border-left:4px solid #2563eb;border-radius:10px;padding:11px 12px;margin-bottom:8px;display:flex;gap:10px;align-items:center;flex-wrap:wrap"><div style="flex:1;min-width:250px"><b style="font-size:12px;color:#0f172a">${escHtml(m.codigoActa)}</b><span style="display:inline-block;margin-left:7px;padding:3px 7px;border-radius:99px;background:#dbeafe;color:#1d4ed8;font-size:8px;font-weight:900">EN TRÁNSITO</span>${m.estadoDocumental==='PENDIENTE_SUBSANAR'?'<span style="display:inline-block;margin-left:5px;padding:3px 7px;border-radius:99px;background:#ffedd5;color:#c2410c;font-size:8px;font-weight:900">GUÍA PENDIENTE</span>':''}<div style="font-size:10px;color:#475569;margin-top:4px">Destino: ${escHtml(m.provinciaDestino)} · ${escHtml(m.ciudadDestino)} · ${escHtml(m.proyectoDestino)}${m.puestoDestino?' · '+escHtml(m.puestoDestino):''}</div><div style="font-size:9px;color:#94a3b8">Responsable: ${escHtml(m.responsable)} · Series: ${(m.armas||[]).map(a=>escHtml(a.serie)).join(', ')}</div></div>${m.urlGuia?`<a href="${escAttr(m.urlGuia)}" target="_blank" rel="noopener" style="border-radius:7px;background:#ecfdf5;color:#047857;padding:7px 9px;font-size:10px;font-weight:900;text-decoration:none">Ver guía</a>`:''}<button onclick="confirmarLlegadaMovimiento('${escAttr(m.codigoActa)}',${Number((m.armas||[]).length)})" style="border:0;border-radius:7px;background:#2563eb;color:white;padding:7px 10px;font-size:10px;font-weight:900;cursor:pointer">✓ Confirmar llegada</button></div>`).join('');
    }catch(e){c.innerHTML=`<p style="color:#b91c1c;font-weight:700">${escHtml(e.message||String(e))}</p>`;}
}
async function confirmarLlegadaMovimiento(codigoActa,cantidad){
    if(!confirm(`¿Confirmas que las ${cantidad} arma(s) de ${codigoActa} llegaron físicamente al proyecto y puesto de destino?\n\nAl continuar pasarán de EN TRÁNSITO a EN CAMPO.`))return;
    try{progresoActa('Confirmando recepción del armamento…',45);const r=await postActas({accion:'confirmar_llegada_armas',token:tokenSesionActual(),codigoActa},90000);if(!r.ok)throw new Error(r.mensaje);progresoActa('Llegada confirmada.',100);alert(r.mensaje);if(typeof cargarDatos==='function')await cargarDatos();await cargarMovimientosTransito();}catch(e){alert(e.message||String(e));}finally{setTimeout(cerrarProgresoActa,350);}
}
...
async function subsanarGuiaPendiente(codigo){
    ...
    try{
        ...
        const r=await postActas({accion:'subsanar_guia_acta',token:tokenSesionActual(),codigo,guia},90000);
    ...
    try{
        ...
        c.innerHTML=['GUARDIA','CUSTODIO'].map(t=>{
            const tarjetas=g[t].map(a=>{
                const invalidada=normalizarTexto(a.estadoActa)==='invalidada';
                const estado=`<span style="display:inline-block;margin-left:7px;padding:3px 7px;border-radius:99px;background:${invalidada?'#fee2e2':'#dcfce7'};color:${invalidada?'#b91c1c':'#15803d'};font-size:8px;font-weight:900">${invalidada?'INVALIDADA':'VIGENTE'}</span>`;
                ...
                return `<div style="background:white;border:1px solid ${invalidada?'#fecaca':'#e2e8f0'};border-radius:10px;padding:10px 12px;margin-bottom:7px;display:flex;gap:10px;align-items:center;flex-wrap:wrap"><div style="flex:1;min-width:220px"><b style="font-size:12px;color:#0f172a">${escHtml(a.codigo)}</b>${estado}${estadoGuia}<div style="font-size:10px;color:#64748b">${escHtml(a.receptor)} · ${a.armas.length} arma(s) · ${escHtml(a.fecha)}</div><div style="font-size:9px;color:#94a3b8">Series: ${a.armas.map(escHtml).join(', ')}</div>${detalleInvalidacion}</div>${a.urlGuia?`<a href="${escAttr(a.urlGuia)}" target="_blank" rel="noopener" style="border:0;border-radius:7px;background:#ecfdf5;color:#047857;padding:6px 8px;font-size:10px;font-weight:900;text-decoration:none">Ver guía</a>`:''}<button onclick="descargarPdfDesdeHistorial('${escAttr(a.codigo)}')" style="border:0;border-radius:7px;background:#dbeafe;color:#075985;padding:6px 8px;font-size:10px;font-weight:900;cursor:pointer">📄 Generar PDF</button>${r.esAdmin?`<button onclick="eliminarUltimaActa('${escAttr(a.codigo)}')" style="border:0;border-radius:7px;background:#fee2e2;color:#b91c1c;padding:6px 8px;font-size:10px;font-weight:900;cursor:pointer">Eliminar</button>`:''}</div>`;
            }).join('');
...
function generarPDFGuardia(d,evidencias){
 ...
 doc.autoTable({startY:y,margin:{left:8,right:8},head:[['N°','CLASE','CATEGORÍA','TIPO','MARCA','MODELO','CALIBRE','SERIE','ALIMENT.','MUNICIONES','PERMISO','COMENTARIO','NOVEDAD']],body:armas.map((a,i)=>[i+1,a.clase||'—',a.categoria||'—',a.tipoArma||'—',a.marca||'—',d.modelo||'—',a.calibre||'—',a.serie||'—',d.alimentadoras||1,d.municiones,d.permiso,d.comentario||'—',d.novedad||'—']),headStyles:{fillColor:[71,85,105],textColor:[255,255,255],fontSize:5.9,halign:'center'},styles:{fontSize:5.8,cellPadding:1.5,halign:'center',valign:'middle'}});y=doc.lastAutoTable.finalY+7;
...
function armasDisponiblesActa(){const orden={rastrillo:0,activo:1,'en campo':1},esCustodio=document.getElementById('acta-tipo')?.value==='custodio';return [...armamentoDetalle].filter(a=>{const estado=normalizarTexto(a?.estado);return a&&(a.serie||a.codigoArma)&&['rastrillo','activo','en campo'].includes(estado)&&(!esCustodio||normalizarTexto(a.categoria).includes('movil'));}).sort((a,b)=>(orden[normalizarTexto(a.estado)]??9)-(orden[normalizarTexto(b.estado)]??9)||String(a.serie||'').localeCompare(String(b.serie||'')));}
