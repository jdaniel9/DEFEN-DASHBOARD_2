...
function respaldoHistorialMovimiento(movimiento){const url=histMovUrl(movimiento?.urlGuia);if(url)return `<a href="${url}" target="_blank" rel="noopener" style="color:#047857;font-weight:900">📄 Ver</a>`;if(movimiento?.rutaGuia)return `<button type="button" onclick="verGuiaHistorialMovimiento('${encodeURIComponent(movimiento.rutaGuia)}')" style="border:0;background:transparent;color:#047857;font-size:9px;font-weight:900;cursor:pointer">📄 Ver</button>`;return '—';}
...
async function consultarHistorialMovimientos(pagina=1){paginaHistorialMovimientos=pagina;const c=document.getElementById('hist-mov-contenido');c.innerHTML='<div style="padding:28px;text-align:center;color:#64748b;font-size:11px">Consultando movimientos…</div>';try{const filtros=filtrosHistorialMovimientos(false),r=backendUsaSupabase()?await supabaseListarHistorialMovimientos(filtros):await postActas(filtros,90000);if(!r.ok)throw new Error(r.mensaje);actualizarCatalogosHistorial(r.catalogos);renderHistorialMovimientos(r);activarGuiasPrivadasHistorial(r.movimientos);insertarRecorridosHistorial(r);insertarResumenVisualHistorial(r);renderPaginacionHistorial(r);}catch(e){c.innerHTML=`<div style="padding:20px;color:#b91c1c;font-weight:800">${histMovEsc(e.message||String(e))}</div>`;}}
