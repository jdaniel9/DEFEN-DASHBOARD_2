...
function renderFiltrosArmamento() {
    ...
    bar.innerHTML = grupos.map(g => `
        <div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;">
            ...
            ${g.valores.map(v => {
                const activo = filtrosArmamento[g.key].includes(v.toLowerCase());
                const colorClass = g.colores && g.colores[v.toLowerCase()] ? g.colores[v.toLowerCase()] : 'active-blue';
                return `<button onclick="toggleFiltroArmamento('${g.key}','${v.toLowerCase()}',this)"
...
function armaPasaFiltros(a) {
    for (const grupo of Object.keys(filtrosArmamento)) {
        ...
        const valorArma = String(a[grupo] || '').toLowerCase();
