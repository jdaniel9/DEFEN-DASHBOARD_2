...
function histMovUrl(valor) { try { const u=new URL(String(valor||''),location.href),ruta=u.pathname.replace(/\/+$/,'').toLowerCase();if(!['http:','https:'].includes(u.protocol))return '';if(u.hostname.endsWith('supabase.com')&&u.pathname.includes('/dashboard/'))return '';if(u.hostname===location.hostname&&(ruta===location.pathname.replace(/\/+$/,'').toLowerCase()||ruta.endsWith('/defen-dashboard_2')||ruta.endsWith('/index.html')))return '';return histMovEsc(u.href); } catch(_) { return ''; } }
async function verGuiaHistorialMovimiento(rutaCodificada){const ventana=window.open('about:blank','_blank');try{const ruta=decodeURIComponent(rutaCodificada||'');if(!ruta)throw new Error('La guía no está disponible.');const url=await supabaseUrlFirmadaGuiaArmamento(ruta,300);if(ventana){ventana.opener=null;ventana.location.replace(url);}else window.location.href=url;}catch(e){if(ventana)ventana.close();alert(e.message||String(e));}}
function etiquetaGuiaHistorial(m){const t=String(m?.tipoMovimiento||'').toUpperCase();if(t==='RETORNO'||t==='RECUPERACION')return 'GUÍA DE RETORNO';if(t.includes('MANTENIMIENTO_RETORNO'))return 'GUÍA RETORNO MTTO.';if(t.includes('MANTENIMIENTO'))return 'GUÍA ENVÍO MTTO.';return 'GUÍA DE ASIGNACIÓN';}
function respaldoHistorialMovimiento(movimiento){const etiqueta=etiquetaGuiaHistorial(movimiento),detalle=[movimiento?.codigoActa,movimiento?.codigoLote].filter(Boolean).join(' · ');if(movimiento?.rutaGuia)return `<button type="button" title="${histMovEsc(detalle||etiqueta)}" onclick="verGuiaHistorialMovimiento('${encodeURIComponent(movimiento.rutaGuia)}')" style="border:0;background:#dcfce7;color:#047857;border-radius:7px;padding:6px 9px;font-size:9px;font-weight:900;cursor:pointer">📄 ${etiqueta}</button>`;const url=histMovUrl(movimiento?.urlGuia);if(url)return `<a href="${url}" target="_blank" rel="noopener" title="${histMovEsc(detalle||etiqueta)}" style="color:#047857;font-weight:900">📄 ${etiqueta}</a>`;return '<span style="color:#94a3b8;font-size:8px;font-weight:800">SIN GUÍA ASOCIADA</span>';}
...
function mejorarRecorridosHistorial(datos){
    ...
    const series=datos.seriesPagina||Object.keys(grupos).sort((a,b)=>(duraciones.get(b)?.diasFueraActual||0)-(duraciones.get(a)?.diasFueraActual||0)||a.localeCompare(b)).slice(0,12);
    [...seccion.querySelectorAll('article')].forEach((articulo,indice)=>{
        const eventos=[...(articulo.children[1]?.children||[])];
        const lista=eventosRecorridoHistorial(grupos[series[indice]]||[]);
...
async function cambiarVistaHistorialArmamento(vista){
    ...
    if(movimientos)await consultarHistorialMovimientos(1);else await renderHistorialMantenimientosV2();
}
...
function insertarPanelGerencialHistorial(){
    ...
    const s=datos.summary||{},armas=datos.weapons||[],afuera=armas.filter(a=>a.state!=='RASTRILLO').sort((a,b)=>Number(b.current_out_days)-Number(a.current_out_days)),sinSalida=armas.filter(a=>Number(a.deployment_count)===0);
    const panel=document.createElement('section');panel.style.cssText='margin-bottom:14px;font-size:11px';
    const kpis=[['TOTAL ARMAS',s.total_weapons||0,'#1d4ed8'],['HAN SALIDO',s.ever_deployed||0,'#15803d'],['SIN SALIDA REGISTRADA',s.never_deployed||0,'#475569'],['FUERA DEL RASTRILLO',s.currently_out||0,'#c2410c'],['PROMEDIO ACUMULADO',`${s.average_out_days||0} DÍAS`,'#7c3aed']];
    panel.innerHTML=`<div style="display:grid;grid-template-columns:repeat(5,minmax(120px,1fr));gap:8px;margin-bottom:10px">${kpis.map(k=>`<div style="background:white;border:1px solid #e2e8f0;border-top:4px solid ${k[2]};border-radius:12px;padding:11px 12px"><small style="font-weight:900;color:#64748b">${k[0]}</small><b style="display:block;font-size:20px;color:${k[2]};margin-top:3px">${k[1]}</b></div>`).join('')}</div><div style="display:grid;grid-template-columns:2fr 1fr;gap:10px"><div style="background:white;border:1px solid #fed7aa;border-radius:13px;padding:12px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:9px"><div><b style="font-size:13px;color:#9a3412">ARMAS FUERA DEL RASTRILLO</b><div style="color:#64748b;margin-top:2px">Selecciona una tarjeta para consultar su recorrido.</div></div><span style="background:#ffedd5;color:#c2410c;border-radius:99px;padding:5px 9px;font-weight:900">${afuera.length}</span></div><div style="display:grid;grid-template-columns:repeat(2,minmax(240px,1fr));gap:7px;max-height:235px;overflow:auto">${afuera.slice(0,30).map(a=>`<button onclick="filtrarRecorridoArma('${encodeURIComponent(a.serial_number)}')" style="display:flex;align-items:center;gap:9px;text-align:left;border:1px solid #fed7aa;background:#fffaf5;border-radius:10px;padding:9px;cursor:pointer"><span style="width:34px;height:34px;display:grid;place-items:center;border-radius:50%;background:#f97316;color:white;font-weight:900">${Number(a.current_out_days)||0}</span><span><b style="display:block;color:#0f172a">SERIE ${histMovEsc(a.serial_number)}</b><small style="color:#64748b">${histMovEsc([a.project,a.post].filter(Boolean).join(' · ')||a.state)} · ${Number(a.current_out_days)||0} día(s)</small></span></button>`).join('')||'<div style="padding:18px;color:#15803d;font-weight:800">Todas las armas se encuentran en el rastrillo.</div>'}</div></div><div style="background:white;border:1px solid #cbd5e1;border-radius:13px;padding:12px"><b style="font-size:13px;color:#334155">SIN SALIDA REGISTRADA</b><div style="font-size:28px;font-weight:950;color:#475569;margin:8px 0">${sinSalida.length}</div><p style="color:#64748b;line-height:1.45">Son armas sin una asignación o salida registrada en Supabase. No significa necesariamente que nunca salieron antes de la migración.</p><button onclick="mostrarArmasSinSalidaHistorial()" style="width:100%;border:0;border-radius:8px;background:#e2e8f0;color:#334155;padding:9px;font-weight:900;cursor:pointer">VER LISTADO</button></div></div>`;
    const recorrido=c.querySelector('section');if(recorrido)recorrido.insertAdjacentElement('afterend',panel);else c.insertBefore(panel,c.firstChild);
}
function mostrarArmasSinSalidaHistorial(){
    const armas=(indicadoresHistorialArmamento?.weapons||[]).filter(a=>Number(a.deployment_count)===0),editor=document.createElement('div');editor.style.cssText='position:fixed;inset:0;z-index:24000;background:rgba(15,23,42,.82);display:grid;place-items:center;padding:20px';
    editor.innerHTML=`<div style="width:min(920px,96vw);max-height:82vh;display:flex;flex-direction:column;background:white;border-radius:16px;overflow:hidden"><header style="padding:15px 18px;background:#0f172a;color:white;display:flex;align-items:center"><div style="flex:1"><b style="font-size:15px">Armas sin salida registrada en Supabase</b><div style="font-size:11px;color:#cbd5e1">${armas.length} arma(s) · listado informativo</div></div><button style="border:0;background:#334155;color:white;border-radius:8px;padding:8px 11px;cursor:pointer">✕ Cerrar</button></header><input placeholder="Buscar serie, tipo o código…" style="margin:12px 14px 6px;padding:10px;border:1px solid #cbd5e1;border-radius:8px"><div style="overflow:auto;padding:8px 14px 15px;display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:7px"></div></div>`;
    const lista=editor.querySelector('div[style*="grid-template-columns"]'),input=editor.querySelector('input'),render=()=>{const q=input.value.trim().toUpperCase(),filas=armas.filter(a=>[a.serial_number,a.weapon_code,a.weapon_type].some(v=>String(v||'').toUpperCase().includes(q)));lista.innerHTML=filas.map(a=>`<div style="border:1px solid #e2e8f0;border-radius:9px;padding:9px"><b>${histMovEsc(a.serial_number)}</b><br><small>${histMovEsc(a.weapon_code||'')} · ${histMovEsc(a.weapon_type||'ARMA')} · ${histMovEsc(a.state||'')}</small></div>`).join('')||'Sin coincidencias.';};
    input.addEventListener('input',render);editor.querySelector('button').onclick=()=>editor.remove();editor.addEventListener('click',e=>{if(e.target===editor)editor.remove();});document.body.appendChild(editor);render();input.focus();
}
...
function fechaBreveHistorial(valor){if(!valor)return'PENDIENTE';const d=new Date(valor);return Number.isFinite(d.getTime())?d.toLocaleDateString('es-EC',{day:'2-digit',month:'short',year:'numeric'}):String(valor);}
function lineaTiempoMantenimientoHtml(m){
    const externo=m.modalidad==='EXTERNO';
    const etapas=externo?[
      ['NOVEDAD REPORTADA',m.fechaReporte,'◆'],['SALIDA AL ARMERO',m.fechaSalida,'🚚'],['RECIBIDA POR ARMERO',m.fechaRecepcionArmero,'●'],['DIAGNÓSTICO',m.fechaFinalizacion,'🔧'],['RETORNO INICIADO',m.fechaRetornoInicio,'↩'],['RECIBIDA EN RASTRILLO',m.fechaRetornoRastrillo,'✓']
    ]:[['NOVEDAD REPORTADA',m.fechaReporte,'◆'],['MANTENIMIENTO INICIADO',m.fechaSalida,'🔧'],['DIAGNÓSTICO',m.fechaFinalizacion,'●'],['FINALIZADO',m.fechaFinalizacion,'✓']];
    return `<div style="display:flex;overflow:auto;padding:14px 2px 6px">${etapas.map((e,i)=>{const listo=Boolean(e[1]);return `<div style="min-width:${externo?'145':'180'}px;position:relative;padding-right:12px"><div style="position:absolute;top:16px;left:34px;right:0;height:3px;background:${i===etapas.length-1?'transparent':listo?'#8b5cf6':'#dbe3ef'}"></div><div style="position:relative;width:34px;height:34px;border-radius:50%;display:grid;place-items:center;background:${listo?'#7c3aed':'#e2e8f0'};color:${listo?'white':'#94a3b8'};font-weight:900;box-shadow:0 0 0 4px white">${e[2]}</div><b style="display:block;font-size:10px;color:${listo?'#334155':'#94a3b8'};margin-top:7px">${e[0]}</b><span style="font-size:10px;color:#64748b">${fechaBreveHistorial(e[1])}</span></div>`;}).join('')}</div>`;
}
async function renderHistorialMantenimientosV2(){
    const c=document.getElementById('hist-mov-contenido');c.innerHTML='<div style="padding:28px;text-align:center">Cargando mantenimientos…</div>';
    try{
        const [workspace,indicadores]=await Promise.all([cargarWorkspaceMantenimientoArmamentoSupabase(),cargarIndicadoresHistorialArmamento()]),todos=workspace?.mantenimientos||[],q=busquedaHistorialMantenimiento.toUpperCase(),filas=todos.filter(m=>!q||[m.serie,m.codigo,m.armeroNombre,m.estadoMantenimiento,m.problema].some(v=>String(v||'').toUpperCase().includes(q))),s=indicadores.summary||{};
        document.getElementById('hist-mov-contador').textContent=`${todos.length} mantenimiento(s) registrados · ${s.maintenance_open||0} abierto(s)`;
        const tarjetas=filas.map(m=>{const arma=(indicadores.weapons||[]).find(a=>a.serial_number===m.serie)||{},total=diasEntreHistorial(m.fechaSalida||m.fechaReporte,m.fechaRetornoRastrillo||m.fechaFinalizacion);return `<article style="background:white;border:1px solid #ddd6fe;border-radius:15px;padding:14px;box-shadow:0 5px 16px rgba(76,29,149,.06)"><header style="display:flex;align-items:flex-start;gap:10px"><div style="flex:1"><h3 style="margin:0;font-size:15px;color:#0f172a">SERIE ${histMovEsc(m.serie||'—')}</h3><p style="margin:4px 0;color:#64748b;font-size:11px"><b>${histMovEsc(m.codigo||'')}</b> · ${histMovEsc(m.modalidad||'')} · ${histMovEsc(m.tipoServicio||'')}</p></div><div style="text-align:right"><span style="display:inline-block;background:#ede9fe;color:#6d28d9;border-radius:99px;padding:5px 9px;font-size:10px;font-weight:900">${histMovEsc(m.estadoMantenimiento||'—')}</span><b style="display:block;color:#7c3aed;font-size:12px;margin-top:5px">${total} DÍA(S)</b></div></header>${lineaTiempoMantenimientoHtml(m)}<details style="margin-top:8px"><summary style="cursor:pointer;background:#f5f3ff;border-radius:8px;padding:8px 10px;color:#6d28d9;font-weight:900;font-size:11px">VER DIAGNÓSTICO, RESULTADO Y DOCUMENTOS</summary><div style="padding:10px 4px 2px;font-size:11px;line-height:1.55"><div style="display:grid;grid-template-columns:repeat(2,minmax(220px,1fr));gap:8px"><div><b>Armero / técnico</b><br>${histMovEsc(m.armeroNombre||'—')} ${m.armeroUbicacion?'· '+histMovEsc(m.armeroUbicacion):''}</div><div><b>Historial acumulado</b><br>${arma.maintenance_count||0} mantenimiento(s) · ${arma.total_maintenance_days||0} día(s)<br>Con armero: ${arma.total_armorer_days||0} día(s)</div><div><b>Problema reportado</b><br>${histMovEsc(m.problema||'—')}</div><div><b>Diagnóstico / resultado</b><br>${histMovEsc(m.diagnostico||'PENDIENTE')} · ${histMovEsc(m.resultado||'PENDIENTE')}</div></div><div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:9px">${m.rutaGuiaEnvio?`<button onclick="verGuiaHistorialMovimiento('${encodeURIComponent(m.rutaGuiaEnvio)}')" style="border:0;border-radius:7px;padding:7px 10px;background:#dbeafe;color:#1d4ed8;font-weight:900;cursor:pointer">📄 GUÍA DE ENVÍO</button>`:''}${m.rutaGuiaRetorno?`<button onclick="verGuiaHistorialMovimiento('${encodeURIComponent(m.rutaGuiaRetorno)}')" style="border:0;border-radius:7px;padding:7px 10px;background:#fef3c7;color:#92400e;font-weight:900;cursor:pointer">📄 GUÍA DE RETORNO</button>`:''}</div></div></details></article>`;}).join('');
        c.innerHTML=`<div style="display:flex;gap:10px;align-items:center;margin-bottom:11px"><div style="flex:1;padding:11px 13px;border-left:4px solid #7c3aed;background:#f5f3ff;border-radius:9px;font-size:11px"><b>Línea de tiempo de mantenimiento</b><br>Cada contenedor muestra el avance del arma desde la novedad hasta su retorno.</div><input id="hist-mtto-buscar" value="${histMovEsc(busquedaHistorialMantenimiento)}" placeholder="Buscar serie, código o armero…" style="width:min(430px,42%);padding:11px;border:1px solid #cbd5e1;border-radius:9px;font-size:11px"></div><div style="display:grid;grid-template-columns:repeat(4,minmax(130px,1fr));gap:8px;margin-bottom:12px">${[['TOTAL',s.maintenance_total||0],['ABIERTOS',s.maintenance_open||0],['ARMAS ATENDIDAS',(indicadores.weapons||[]).filter(a=>Number(a.maintenance_count)>0).length],['PROMEDIO',`${s.average_maintenance_days||0} DÍAS`]].map(k=>`<div style="background:white;border:1px solid #ddd6fe;border-top:4px solid #7c3aed;border-radius:11px;padding:10px"><small style="font-weight:900;color:#64748b">${k[0]}</small><b style="display:block;font-size:20px;color:#6d28d9;margin-top:3px">${k[1]}</b></div>`).join('')}</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(470px,1fr));gap:11px">${tarjetas||'<div style="padding:30px">No existen mantenimientos con esta búsqueda.</div>'}</div>`;
        const buscar=document.getElementById('hist-mtto-buscar');buscar?.addEventListener('keydown',e=>{if(e.key==='Enter'){busquedaHistorialMantenimiento=e.target.value.trim();renderHistorialMantenimientosV2();}});
    }catch(e){c.innerHTML=`<div style="padding:20px;color:#b91c1c;font-weight:800">${histMovEsc(e.message||String(e))}</div>`;}
}
function etiquetaMovimientoHistorial(tipo){return ({ENTREGA_REAL:'Entrega real',ASIGNACION:'Asignación',REASIGNACION:'Reasignación',TRASLADO:'Traslado',RETORNO:'Retorno',REGULARIZACION:'Regularización',PERDIDA:'Pérdida/Robada',CONFISCACION:'Confiscación',RECUPERACION:'Recuperación',NOVEDAD_TECNICA:'Novedad técnica',MANTENIMIENTO_LOCAL:'Mantenimiento local',MANTENIMIENTO_SALIDA:'Envío al armero',MANTENIMIENTO_ENVIO:'Envío al armero',MANTENIMIENTO_RESULTADO:'Resultado de mantenimiento',MANTENIMIENTO_RETORNO:'Retorno de mantenimiento'}[tipo]||tipo||'Movimiento');}
function renderHistorialMovimientos(datos){const c=document.getElementById('hist-mov-contenido'),movimientos=datos.movimientos||[];historialMovimientosActual=movimientos;document.getElementById('hist-mov-contador').textContent=`${datos.total||0} movimiento(s) encontrado(s) · página ${datos.pagina||1} de ${datos.totalPaginas||1}`;if(!movimientos.length){c.innerHTML='<div style="padding:28px;text-align:center;color:#94a3b8;font-size:11px">No existen movimientos con estos filtros.</div>';return;}c.innerHTML=`<div style="overflow:auto;background:white;border:1px solid #e2e8f0;border-radius:12px"><table style="width:100%;border-collapse:collapse;font-size:9px;min-width:1180px"><thead style="position:sticky;top:0;background:#0f172a;color:white;z-index:2"><tr><th style="padding:8px">Fecha</th><th>Serie</th><th>Movimiento</th><th>Cambio de estado</th><th>Origen</th><th>Destino</th><th>Responsable</th><th>Usuario</th><th>Acta</th><th>Respaldo</th><th>Detalle</th></tr></thead><tbody>${movimientos.map((m,i)=>`<tr style="border-bottom:1px solid #e2e8f0;${i%2?'background:#f8fafc':''}"><td style="padding:8px;white-space:nowrap">${histMovEsc(m.fechaEfectiva||m.fechaRegistro||'—')}</td><td style="padding:6px;font-weight:900">${histMovEsc(m.serie||'—')}<br><span style="color:#94a3b8;font-weight:600">${histMovEsc(m.codigoArma||'')}</span></td><td style="padding:6px"><span style="background:#dbeafe;color:#1d4ed8;border-radius:99px;padding:3px 7px;font-weight:900;white-space:nowrap">${histMovEsc(etiquetaMovimientoHistorial(m.tipoMovimiento))}</span><br><span style="color:#64748b">${histMovEsc(m.estadoMovimiento)}</span></td><td style="padding:6px">${histMovEsc(m.estadoAnterior||'—')} → <b>${histMovEsc(m.estadoNuevo||'—')}</b></td><td style="padding:6px">${histMovEsc([m.provinciaOrigen,m.proyectoOrigen,m.puestoOrigen].filter(Boolean).join(' · ')||'—')}</td><td style="padding:6px">${histMovEsc([m.provinciaDestino,m.proyectoDestino,m.puestoDestino].filter(Boolean).join(' · ')||'—')}</td><td style="padding:6px">${histMovEsc(m.responsableNuevo||m.responsableAnterior||'—')}</td><td style="padding:6px">${histMovEsc(m.usuarioRegistra||'—')}<br><span style="color:#94a3b8">${histMovEsc(m.fechaRegistro||'')}</span></td><td style="padding:6px">${histMovEsc(m.codigoActa||'—')}</td><td style="padding:6px;text-align:center">${respaldoHistorialMovimiento(m)}</td><td style="padding:6px;text-align:center"><button onclick="alternarDetalleMovimiento(${i})" style="border:0;border-radius:6px;background:#e2e8f0;color:#334155;padding:5px 7px;font-size:8px;font-weight:900;cursor:pointer">Ver</button></td></tr><tr id="hist-mov-detalle-${i}" style="display:none;background:#eff6ff"><td colspan="11" style="padding:10px 14px"><div style="display:grid;grid-template-columns:repeat(3,minmax(220px,1fr));gap:10px"><div><b>Origen completo</b><br>${histMovEsc([m.provinciaOrigen,m.ciudadOrigen,m.proyectoOrigen,m.puestoOrigen,m.ubicacionOrigen].filter(Boolean).join(' · ')||'—')}</div><div><b>Destino completo</b><br>${histMovEsc([m.provinciaDestino,m.ciudadDestino,m.proyectoDestino,m.puestoDestino,m.ubicacionDestino].filter(Boolean).join(' · ')||'—')}</div><div><b>Responsabilidad</b><br>Anterior: ${histMovEsc(m.responsableAnterior||'—')} ${m.cedulaAnterior?'· '+histMovEsc(m.cedulaAnterior):''}<br>Nueva: ${histMovEsc(m.responsableNuevo||'—')} ${m.cedulaNuevo?'· '+histMovEsc(m.cedulaNuevo):''}</div><div><b>Lote / movimiento</b><br>${histMovEsc(m.loteId||'—')}<br>${histMovEsc(m.idMovimiento||'')}</div><div><b>Recepción</b><br>${histMovEsc(m.fechaRecepcion||'Pendiente')} ${m.usuarioRecepciona?'· '+histMovEsc(m.usuarioRecepciona):''}</div><div><b>Observación</b><br>${histMovEsc(m.observacion||'Sin observación')}</div></div></td></tr>`).join('')}</tbody></table></div>`;}
...
function iconoMovimientoHistorial(tipo){return ({ENTREGA_REAL:'↗',ASIGNACION:'↗',REGULARIZACION:'✓',RETORNO:'↩',RECUPERACION:'♻',PERDIDA:'!',CONFISCACION:'⚠',NOVEDAD_TECNICA:'◆',MANTENIMIENTO_LOCAL:'🔧',MANTENIMIENTO_ENVIO:'🚚',MANTENIMIENTO_RESULTADO:'✓',MANTENIMIENTO_RETORNO:'↩'}[tipo]||'•');}
function colorMovimientoHistorial(tipo){if(String(tipo).includes('MANTENIMIENTO'))return '#7c3aed';if(['RETORNO','RECUPERACION'].includes(tipo))return '#0f766e';if(['PERDIDA','CONFISCACION'].includes(tipo))return '#dc2626';if(tipo==='ENTREGA_REAL')return '#2563eb';if(tipo==='REGULARIZACION')return '#ea580c';return '#2563eb';}
function aplicarContextoRegularizacionesHistorial(datos,contextos){const mapa=new Map((contextos||[]).map(x=>[`${x.act_code}||${x.serial_number}`,x]));(datos.movimientos||[]).forEach(m=>{if(m.tipoMovimiento!=='REGULARIZACION')return;const x=mapa.get(`${m.codigoActa}||${m.serie}`);if(x){m.entregaReal=x.delivery_at;m.fechaRegularizacion=x.regularized_on;m.fechaRegistroRegularizacion=x.registered_at;m.fechaRetornoReal=x.returned_at;}});}
function eventosRecorridoHistorial(lista){const salida=[];lista.slice().sort((a,b)=>String(a.fechaEfectiva||a.fechaRegistro).localeCompare(String(b.fechaEfectiva||b.fechaRegistro))).forEach(m=>{if(m.tipoMovimiento==='REGULARIZACION'&&m.entregaReal){salida.push({...m,tipoMovimiento:'ENTREGA_REAL',fechaEfectiva:m.entregaReal,rutaGuia:null,urlGuia:null,codigoActa:null,codigoLote:null,estadoNuevo:'ACTIVO'});salida.push({...m,fechaEfectiva:m.fechaRegularizacion||m.fechaRegistroRegularizacion||m.fechaRegistro});}else if(m.tipoMovimiento==='RETORNO')salida.push({...m,fechaEfectiva:m.fechaRecepcion||m.fechaEfectiva});else salida.push(m);});return salida;}
function duracionMesesDiasHistorial(inicio,fin){if(!inicio)return 'SIN FECHA';const a=new Date(inicio),b=fin?new Date(fin):new Date();if(!Number.isFinite(a.getTime())||!Number.isFinite(b.getTime())||b<a)return 'SIN DURACIÓN';let meses=(b.getUTCFullYear()-a.getUTCFullYear())*12+b.getUTCMonth()-a.getUTCMonth(),cursor=new Date(a);cursor.setUTCMonth(cursor.getUTCMonth()+meses);if(cursor>b){meses--;cursor=new Date(a);cursor.setUTCMonth(cursor.getUTCMonth()+meses);}const dias=Math.max(0,Math.floor((b-cursor)/86400000));return `${meses} ${meses===1?'MES':'MESES'} · ${dias} ${dias===1?'DÍA':'DÍAS'}`;}
function insertarRecorridosHistorialV3(datos){
    const c=document.getElementById('hist-mov-contenido'),movs=datos.movimientos||[];if(!c||!movs.length)return;const grupos={};movs.forEach(m=>(grupos[m.serie]||(grupos[m.serie]=[])).push(m));const duraciones=new Map((datos.resumen?.duraciones||[]).map(x=>[x.serie,x])),todasSeries=Object.keys(grupos).sort((a,b)=>(duraciones.get(b)?.diasFueraActual||0)-(duraciones.get(a)?.diasFueraActual||0)||a.localeCompare(b)),porPagina=12,totalPaginas=Math.max(1,Math.ceil(todasSeries.length/porPagina)),pagina=Math.min(Math.max(1,paginaHistorialMovimientos),totalPaginas),series=todasSeries.slice((pagina-1)*porPagina,pagina*porPagina);paginaHistorialMovimientos=pagina;datos.pagina=pagina;datos.totalPaginasSeries=totalPaginas;datos.totalSeries=todasSeries.length;datos.seriesPagina=series;const contador=document.getElementById('hist-mov-contador');if(contador)contador.textContent=`${movs.length} movimiento(s) · ${todasSeries.length} arma(s) · página ${pagina} de ${totalPaginas}`;
    const html=series.map(serie=>{const originales=grupos[serie],lista=eventosRecorridoHistorial(originales),ultimo=lista[lista.length-1],d=duraciones.get(serie)||{},actual=d.diasFueraActual>0,reg=originales.find(m=>m.tipoMovimiento==='REGULARIZACION'&&m.entregaReal),entrega=reg?.entregaReal,retorno=reg?.fechaRetornoReal||null,ubicacion=[ultimo.provinciaDestino,ultimo.proyectoDestino,ultimo.puestoDestino].filter(Boolean).join(' · ')||ultimo.ubicacionDestino||ultimo.estadoNuevo||'SIN UBICACIÓN',duracion=entrega?duracionMesesDiasHistorial(entrega,retorno):'';return `<article style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:12px;box-shadow:0 4px 14px rgba(15,23,42,.05)"><header style="display:flex;align-items:flex-start;gap:10px"><div style="flex:1"><button onclick="filtrarRecorridoArma('${encodeURIComponent(serie)}')" style="border:0;background:none;padding:0;color:#0f172a;font-size:12px;font-weight:950;cursor:pointer">SERIE ${histMovEsc(serie)}</button><div style="font-size:8px;color:#64748b;margin-top:3px">${histMovEsc(ubicacion)}</div></div><span style="background:${actual?'#fff7ed':'#ecfdf5'};color:${actual?'#c2410c':'#047857'};border-radius:99px;padding:5px 8px;font-size:8px;font-weight:950;white-space:nowrap">${duracion|| (actual?d.diasFueraActual+' DÍAS AFUERA':histMovEsc(ultimo.estadoNuevo||'REGISTRADA'))}</span></header><div style="display:flex;align-items:flex-start;overflow:auto;padding:13px 2px 4px;margin-top:3px">${lista.map((m,i)=>{const color=colorMovimientoHistorial(m.tipoMovimiento);return `<div style="min-width:128px;position:relative;padding-right:9px"><div style="position:absolute;top:13px;left:22px;right:-3px;height:2px;background:${i===lista.length-1?'transparent':'#cbd5e1'}"></div><div style="position:relative;width:28px;height:28px;border-radius:50%;background:${color};color:white;display:grid;place-items:center;font-size:11px;font-weight:900;box-shadow:0 0 0 4px white">${iconoMovimientoHistorial(m.tipoMovimiento)}</div><div style="font-size:8px;font-weight:950;color:#334155;margin-top:7px">${histMovEsc(etiquetaMovimientoHistorial(m.tipoMovimiento))}</div><div style="font-size:7px;color:#64748b;margin-top:2px">${histMovEsc(m.fechaEfectiva||'SIN FECHA')}</div><div style="font-size:7px;color:#94a3b8;margin-top:2px">${histMovEsc(m.estadoNuevo||'')}</div>${m.tipoMovimiento==='ENTREGA_REAL'?'':respaldoHistorialMovimiento(m)}</div>`;}).join('')}</div>${entrega?`<details style="margin-top:8px"><summary style="cursor:pointer;background:#fff7ed;color:#c2410c;border-radius:8px;padding:7px 9px;font-size:9px;font-weight:900">VER TIEMPO FUERA DEL RASTRILLO</summary><div style="padding:8px;font-size:9px;line-height:1.6"><b>Entrega real:</b> ${histMovEsc(entrega)}<br><b>Regularización:</b> ${histMovEsc(reg.fechaRegularizacion||reg.fechaRegistroRegularizacion||'—')}<br><b>Retorno:</b> ${histMovEsc(retorno||'AÚN FUERA DEL RASTRILLO')}<br><b>Duración:</b> ${histMovEsc(duracion)}</div></details>`:''}</article>`;}).join('');
    c.insertAdjacentHTML('afterbegin',`<section style="margin-bottom:12px"><div style="display:flex;align-items:center;gap:8px;margin:2px 0 8px"><div style="flex:1"><b style="font-size:11px;color:#0f172a">RECORRIDO OPERATIVO POR ARMA</b><div style="font-size:8px;color:#64748b">Entrega real, regularización, movimientos posteriores y retorno</div></div><span style="font-size:8px;color:#94a3b8">Mostrando ${series.length} de ${todasSeries.length} arma(s) · página ${pagina} de ${totalPaginas}</span></div><div style="display:grid;grid-template-columns:repeat(2,minmax(360px,1fr));gap:9px">${html}</div><details class="hist-tabla-tecnica" style="margin-top:10px"><summary style="cursor:pointer;background:#e2e8f0;color:#334155;border-radius:9px;padding:8px 11px;font-size:9px;font-weight:900">VER TABLA TÉCNICA COMPLETA</summary></details></section>`);const details=c.querySelector('details.hist-tabla-tecnica');const tabla=[...c.children].find(x=>x.matches?.('div')&&x.querySelector?.('table'));if(details&&tabla){details.appendChild(tabla);details.addEventListener('toggle',()=>{if(details.open){agregarDatosTecnicosHistorialMovimientos();agregarAccionesAdminHistorialMovimientos();}});}
}
function insertarRecorridosHistorial(datos){
    const c=document.getElementById('hist-mov-contenido'),movs=datos.movimientos||[];if(!c||!movs.length)return;const grupos={};movs.forEach(m=>(grupos[m.serie]||(grupos[m.serie]=[])).push(m));const duraciones=new Map((datos.resumen?.duraciones||[]).map(x=>[x.serie,x])),todasSeries=Object.keys(grupos).sort((a,b)=>(duraciones.get(b)?.diasFueraActual||0)-(duraciones.get(a)?.diasFueraActual||0)||a.localeCompare(b)),porPagina=12,totalPaginas=Math.max(1,Math.ceil(todasSeries.length/porPagina)),pagina=Math.min(Math.max(1,paginaHistorialMovimientos),totalPaginas),series=todasSeries.slice((pagina-1)*porPagina,pagina*porPagina);paginaHistorialMovimientos=pagina;datos.pagina=pagina;datos.totalPaginasSeries=totalPaginas;datos.totalSeries=todasSeries.length;datos.seriesPagina=series;const contador=document.getElementById('hist-mov-contador');if(contador)contador.textContent=`${movs.length} movimiento(s) · ${todasSeries.length} arma(s) · página ${pagina} de ${totalPaginas}`;
    const html=series.map(serie=>{const lista=grupos[serie].slice().sort((a,b)=>String(a.fechaEfectiva||a.fechaRegistro).localeCompare(String(b.fechaEfectiva||b.fechaRegistro))),ultimo=lista[lista.length-1],d=duraciones.get(serie)||{},actual=d.diasFueraActual>0,ubicacion=[ultimo.provinciaDestino,ultimo.proyectoDestino,ultimo.puestoDestino].filter(Boolean).join(' · ')||ultimo.ubicacionDestino||ultimo.estadoNuevo||'SIN UBICACIÓN';return `<article style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:12px;box-shadow:0 4px 14px rgba(15,23,42,.05)"><header style="display:flex;align-items:flex-start;gap:10px"><div style="flex:1"><button onclick="filtrarRecorridoArma('${encodeURIComponent(serie)}')" style="border:0;background:none;padding:0;color:#0f172a;font-size:12px;font-weight:950;cursor:pointer">SERIE ${histMovEsc(serie)}</button><div style="font-size:8px;color:#64748b;margin-top:3px">${histMovEsc(ubicacion)}</div></div><span style="background:${actual?'#fff7ed':'#ecfdf5'};color:${actual?'#c2410c':'#047857'};border-radius:99px;padding:5px 8px;font-size:8px;font-weight:950;white-space:nowrap">${actual?d.diasFueraActual+' DÍAS AFUERA':histMovEsc(ultimo.estadoNuevo||'REGISTRADA')}</span></header><div style="display:flex;align-items:flex-start;overflow:auto;padding:13px 2px 4px;margin-top:3px">${lista.map((m,i)=>{const color=colorMovimientoHistorial(m.tipoMovimiento);return `<div style="min-width:128px;position:relative;padding-right:9px"><div style="position:absolute;top:13px;left:22px;right:-3px;height:2px;background:${i===lista.length-1?'transparent':'#cbd5e1'}"></div><div style="position:relative;width:28px;height:28px;border-radius:50%;background:${color};color:white;display:grid;place-items:center;font-size:11px;font-weight:900;box-shadow:0 0 0 4px white">${iconoMovimientoHistorial(m.tipoMovimiento)}</div><div style="font-size:8px;font-weight:950;color:#334155;margin-top:7px">${histMovEsc(etiquetaMovimientoHistorial(m.tipoMovimiento))}</div><div style="font-size:7px;color:#64748b;margin-top:2px">${histMovEsc(m.fechaEfectiva||'SIN FECHA')}</div><div style="font-size:7px;color:#94a3b8;margin-top:2px">${histMovEsc(m.estadoNuevo||'')}</div>${respaldoHistorialMovimiento(m)}</div>`;}).join('')}</div></article>`;}).join('');
    c.insertAdjacentHTML('afterbegin',`<section style="margin-bottom:12px"><div style="display:flex;align-items:center;gap:8px;margin:2px 0 8px"><div style="flex:1"><b style="font-size:11px;color:#0f172a">RECORRIDO OPERATIVO POR ARMA</b><div style="font-size:8px;color:#64748b">Línea de tiempo desde la salida, asignación, mantenimientos y retorno</div></div><span style="font-size:8px;color:#94a3b8">Mostrando ${series.length} de ${todasSeries.length} arma(s) · página ${pagina} de ${totalPaginas}</span></div><div style="display:grid;grid-template-columns:repeat(2,minmax(360px,1fr));gap:9px">${html}</div><details style="margin-top:10px"><summary style="cursor:pointer;background:#e2e8f0;color:#334155;border-radius:9px;padding:8px 11px;font-size:9px;font-weight:900">VER TABLA TÉCNICA COMPLETA</summary></details></section>`);
...
async function consultarHistorialMovimientos(pagina=1){
    ...
    try{
        const filtros=filtrosHistorialMovimientos(true),r=backendUsaSupabase()?await supabaseListarHistorialMovimientos(filtros):await postActas(filtros,90000);
        if(!r.ok)throw new Error(r.mensaje);
        if(backendUsaSupabase()){const codigos=[...new Set((r.movimientos||[]).filter(m=>m.tipoMovimiento==='REGULARIZACION'&&m.codigoActa).map(m=>m.codigoActa))];if(codigos.length)aplicarContextoRegularizacionesHistorial(r,await supabaseCargarContextoRegularizaciones(codigos));}
        ...
        insertarRecorridosHistorialV3(r);mejorarRecorridosHistorial(r);insertarPanelGerencialHistorial();renderPaginacionHistorial(r);
    }catch(e){c.innerHTML=`<div style="padding:20px;color:#b91c1c;font-weight:800">${histMovEsc(e.message||String(e))}</div>`;}
}
