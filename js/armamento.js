...
async function abrirMantenimientoArmamento(serieCodificada){
    ...
    try{if(arma.idMantenimientoActual){const r=await postActas({accion:'listar_mantenimientos_armamento',token:tokenSesionActual()},90000);if(!r.ok)throw new Error(r.mensaje);mantenimientoArmaActual.mantenimiento=(r.mantenimientos||[]).find(m=>m.idMantenimiento===arma.idMantenimientoActual)||null;}renderFormularioMantenimiento();}catch(e){c.innerHTML=`<div style="padding:18px;color:#b91c1c;font-weight:800">${histMovEsc(e.message||String(e))}</div>`;}
}
...
async function iniciarRetornoMantenimientoDesdeModal(){try{const guia=await guiaMantenimientoBase64('mtto-guia-retorno'),destino=document.getElementById('mtto-destino').value;if(!destino)return alert('Selecciona el rastrillo de destino.');await completarAccionMantenimiento({accion:'iniciar_retorno_mantenimiento',idSolicitud:`mtto_ret_${Date.now()}_${Math.random().toString(36).slice(2)}`,idMantenimiento:mantenimientoArmaActual.mantenimiento.idMantenimiento,fecha:document.getElementById('mtto-fecha-etapa').value,destino,guia},'¿Iniciar el retorno del arma reparada hacia el rastrillo?');}catch(e){alert(e.message||String(e));}}
...
function asegurarModalHistorialMovimientos() {
    ...
    modal.innerHTML=`<div style="width:98vw;max-width:1450px;height:92vh;background:#f8fafc;border-radius:18px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 30px 90px rgba(0,0,0,.5)"><div style="background:#0f172a;color:white;padding:14px 18px;display:flex;align-items:center;gap:10px"><div style="flex:1"><h2 style="margin:0;font-size:15px;font-weight:900">🧭 Historial de Movimientos de Armamento</h2><p id="hist-mov-contador" style="margin:3px 0 0;color:#94a3b8;font-size:10px">Cargando…</p></div><button onclick="cerrarHistorialMovimientosArmamento()" style="border:0;border-radius:8px;background:rgba(255,255,255,.12);color:white;padding:7px 11px;cursor:pointer">✕ Cerrar</button></div><div style="background:white;padding:11px 14px;border-bottom:1px solid #e2e8f0"><div style="display:grid;grid-template-columns:minmax(180px,2fr) repeat(4,minmax(120px,1fr));gap:7px"><input id="hist-mov-consulta" placeholder="Serie, arma, acta, responsable…" style="padding:8px;border:1px solid #cbd5e1;border-radius:8px;font-size:10px"><select id="hist-mov-tipo" style="padding:8px;border:1px solid #cbd5e1;border-radius:8px;font-size:10px"><option value="">Todos los movimientos</option></select><select id="hist-mov-estado" style="padding:8px;border:1px solid #cbd5e1;border-radius:8px;font-size:10px"><option value="">Todos los estados</option></select><select id="hist-mov-provincia" style="padding:8px;border:1px solid #cbd5e1;border-radius:8px;font-size:10px"><option value="">Todas las provincias</option></select><select id="hist-mov-proyecto" style="padding:8px;border:1px solid #cbd5e1;border-radius:8px;font-size:10px"><option value="">Todos los proyectos</option></select></div><div style="display:flex;gap:7px;align-items:center;flex-wrap:wrap;margin-top:7px"><label style="font-size:9px;font-weight:800;color:#64748b">Desde <input id="hist-mov-desde" type="date" style="padding:7px;border:1px solid #cbd5e1;border-radius:8px;font-size:10px"></label><label style="font-size:9px;font-weight:800;color:#64748b">Hasta <input id="hist-mov-hasta" type="date" style="padding:7px;border:1px solid #cbd5e1;border-radius:8px;font-size:10px"></label><button onclick="consultarHistorialMovimientos(1)" style="border:0;border-radius:8px;background:#0369a1;color:white;padding:8px 12px;font-size:10px;font-weight:900;cursor:pointer">🔎 Consultar</button><button onclick="limpiarFiltrosHistorialMovimientos()" style="border:0;border-radius:8px;background:#e2e8f0;color:#334155;padding:8px 12px;font-size:10px;font-weight:900;cursor:pointer">Limpiar</button><span style="flex:1"></span><button onclick="exportarHistorialMovimientosPDF()" style="border:0;border-radius:8px;background:#d97706;color:white;padding:8px 12px;font-size:10px;font-weight:900;cursor:pointer">PDF</button><button onclick="exportarHistorialMovimientosExcel()" style="border:0;border-radius:8px;background:#15803d;color:white;padding:8px 12px;font-size:10px;font-weight:900;cursor:pointer">Excel</button></div></div><div id="hist-mov-contenido" style="flex:1;overflow:auto;padding:12px 14px"></div><div id="hist-mov-paginacion" style="background:white;border-top:1px solid #e2e8f0;padding:9px 14px;display:flex;justify-content:center;align-items:center;gap:8px"></div></div>`;document.body.appendChild(modal);
...
function agregarAccionesAdminHistorialMovimientos(){
    if(typeof rolActual!=='function'||rolActual()!=='admin')return;
...
async function consultarHistorialMovimientos(pagina=1){paginaHistorialMovimientos=pagina;const c=document.getElementById('hist-mov-contenido');c.innerHTML='<div style="padding:28px;text-align:center;color:#64748b;font-size:11px">Consultando movimientos…</div>';try{const r=await postActas(filtrosHistorialMovimientos(false),90000);if(!r.ok)throw new Error(r.mensaje);actualizarCatalogosHistorial(r.catalogos);renderHistorialMovimientos(r);insertarResumenVisualHistorial(r);agregarDatosTecnicosHistorialMovimientos();agregarAccionesAdminHistorialMovimientos();renderPaginacionHistorial(r);}catch(e){c.innerHTML=`<div style="padding:20px;color:#b91c1c;font-weight:800">${histMovEsc(e.message||String(e))}</div>`;}}
