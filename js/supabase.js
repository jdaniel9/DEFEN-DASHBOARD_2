...
async function supabasePeticion(ruta, opciones = {}) {
    if (!/^\/(?:auth|rest|storage|functions)\/v1\/[A-Za-z0-9_?=&.,%+\/-]*$/.test(String(ruta||''))) throw new Error('RUTA DE API NO PERMITIDA.');
    const metodo=String(opciones.method||'GET').toUpperCase();
    if (!['GET','POST','PATCH','DELETE'].includes(metodo)) throw new Error('MÉTODO HTTP NO PERMITIDO.');
    if (String(ruta).startsWith('/storage/v1/') && metodo!=='GET') DefenSecurity.rateLimit('storage:mutation',20,60);
...
async function supabaseObtenerPerfil(token, userId) {
    DefenSecurity.rateLimit('rpc:get_my_profile_security',30,60);
    const perfil = await supabasePeticion('/rest/v1/rpc/get_my_profile_security', {
        method:'POST',headers:supabaseHeaders(token,true),body:'{}'
    });
    if (!perfil || perfil.id !== userId) throw new Error('No se encontró el perfil interno del usuario.');
    if (!perfil.active) throw new Error('El usuario está desactivado.');
    return perfil;
}

async function supabaseIniciarSesion(usuario, password, departamento) {
    const username = String(usuario || '').trim().toLowerCase();
    DefenSecurity.rateLimit('auth:login',5,300);
    if (!/^[a-z0-9._-]{3,40}$/.test(username)) throw new Error('EL USUARIO NO TIENE UN FORMATO VÁLIDO.');
    if (typeof password!=='string' || password.length<8 || password.length>128) throw new Error('LA CONTRASEÑA NO TIENE UNA LONGITUD VÁLIDA.');
    if (departamento && !/^[a-z_]{3,40}$/.test(String(departamento))) throw new Error('EL DEPARTAMENTO NO ES VÁLIDO.');
...
async function supabaseRpc(nombre, payload = {}) {
    DefenSecurity.rpc(nombre,payload);
...
async function supabaseCargarRadiosDetalle() {
    const workspace=await supabaseRpc('get_radio_management_workspace');
    return (Array.isArray(workspace?.radios) ? workspace.radios : []).map(radio => ({
        id: radio.id || '',
        provinciaId: radio.province_id == null ? '' : String(radio.province_id),
        proyectoId: radio.project_id || '',
        puestoId: radio.post_id || '',
        provincia: String(radio.province || '').toUpperCase().trim(),
        proyecto: radio.project || '',
        puesto: radio.post || '',
        modelo: radio.model || '',
        serie: radio.serial_number || '',
        estado: radio.state || '',
        observacion: radio.observation || '',
        actualizado: radio.updated_at || ''
    }));
}
...
function adaptarSnapshotSupabase(snapshot, directorio = null) {
    ...
    const salida = {
        ...
        __personal_actas__: [],
        __supervisores__: [],
        __personal_contacto__: {}
    };

    const personasPorId = new Map((directorio?.people || []).map(p => [String(p.id), p]));
    const personasPorNombre = new Map((directorio?.people || []).map(p => [String(p.full_name || '').toUpperCase().trim(), p]));
    const asignacionesPorPersona = new Map();
    (directorio?.assignments || []).forEach(a => {
        if (a.personnel_id && !asignacionesPorPersona.has(String(a.personnel_id))) asignacionesPorPersona.set(String(a.personnel_id), a);
    });
    (directorio?.people || []).forEach(p => {
        const asignacion = asignacionesPorPersona.get(String(p.id)) || {};
        const contacto = {
            id:p.id || '', nombre:p.full_name || '', cedula:p.national_id || '', telefono:p.mobile_phone || '',
            fechaIngreso:asignacion.employment_start_date || '', fechaSalida:asignacion.employment_end_date || ''
        };
        salida.__personal_contacto__[String(p.full_name || '').toUpperCase().trim()] = contacto;
        salida.__personal_contacto__[String(p.id)] = contacto;
    });
    ...
    (snapshot.projects || []).forEach(p => {
        ...
        const proyectoLegacy = {
            id: p.id || '', nombre: p.name || '', guardias: Number(p.guards) || 0, armas: Number(p.weapons) || 0,
            radios: Number(p.radios) || 0, puestos: (p.posts || []).length, vacantes: Number(p.vacancies) || 0,
            inicio: (directorio?.projects || []).find(x=>x.id===p.id)?.start_date || p.start_date || '',
        ...
        salida.__supervisores__.push(...(p.supervisors || []).map(s => {
            const persona=personasPorId.get(String(s.personnel_id||''))||personasPorNombre.get(String(s.full_name||'').toUpperCase().trim())||{};
            return { personalId:persona.id||s.personnel_id||'',cedula:persona.national_id||s.national_id||'',nombre:s.full_name||persona.full_name||'',provincia,proyecto:p.name||'',estado:'ACTIVO' };
        }));
...
async function supabaseCargarDashboardLegacy() {
    ...
    const puedeVerLlamados = ['admin', 'operaciones', 'gdp'].includes((sessionStorage.getItem('defen_auth_rol') || '').toLowerCase());
    const [snapshot, radios, workspaceArmamento, workspaceLlamados, directorio] = await Promise.all([
        ...
        puedeVerLlamados && typeof SUPABASE_PERSONNEL_WARNINGS_ENABLED !== 'undefined' && SUPABASE_PERSONNEL_WARNINGS_ENABLED
            ? supabaseCargarWorkspaceLlamadosAtencion().catch(error => { console.warn('Llamados de atención:', error.message); return null; })
            : Promise.resolve(null),
        supabaseCargarDirectorioPersonal().catch(error => { console.warn('Directorio de personal:', error.message); return null; })
    ]);
    const salida = adaptarSnapshotSupabase(snapshot, directorio);
    if (puedeVerRadios) salida.__radios_detalle__ = radios;
    if (workspaceLlamados) salida.__llamados__ = adaptarLlamadosAtencionSupabase(workspaceLlamados.warnings);
...
async function supabasePrevisualizarArchivoProyecto(projectId) {
    return supabaseRpc('preview_project_archive', { p_project_id: projectId });
}

async function supabaseArchivarProyecto(payload) {
    return supabaseRpc('archive_project_application', { p_payload: payload });
}

function adaptarLlamadosAtencionSupabase(filas) {
    return (Array.isArray(filas) ? filas : []).map(w => ({
        id: w.id || '',
        fecha: w.warning_date || '',
        puesto: w.post_name || '',
        proyecto: w.project_name || '',
        provincia: w.province_name || '',
        nombre_guardia: w.personnel_name || '',
        motivo: w.reason || '',
        tipo_llamado: w.warning_type || '',
        registrado_por: w.registered_by_name || ''
    }));
}

async function supabaseCargarWorkspaceLlamadosAtencion(limite = 300) {
    return supabaseRpc('get_personnel_warning_workspace', { p_limit: limite });
}

async function supabaseCrearLlamadoAtencion(payload) {
    return supabaseRpc('create_personnel_warning_application', { p_payload: payload });
}

async function supabaseCargarWorkspaceProyectos() {
    return supabaseRpc('get_project_management_workspace');
}

async function supabaseCargarWorkspacePersonalDotacion() {
    return supabaseRpc('get_personal_duty_workspace');
}

async function supabaseCargarFechasPersonalDotacion() {
    return supabaseRpc('get_personal_duty_dates');
}

async function supabaseGuardarPersonalDotacion(payload) {
    return supabaseRpc('save_personal_duty_assignment', { p_payload: payload });
}

async function supabaseCargarDirectorioPersonal() {
    return supabaseRpc('get_personnel_directory_application');
}

async function supabaseGuardarSupervisor(payload) {
    return supabaseRpc('save_supervisor_profile_application', { p_payload: payload });
}

async function supabaseActualizarContactoPersonal(payload) {
    return supabaseRpc('update_personnel_contact_application', { p_payload: payload });
}

async function supabaseGuardarProyecto(payload) {
    return supabaseRpc('save_project_application', { p_payload: payload });
}

async function supabaseGuardarPuestoProyecto(payload) {
    return supabaseRpc('save_project_post_application', { p_payload: payload });
}

async function supabaseCambiarSupervisorProyecto(payload) {
    return supabaseRpc('set_project_supervisor_application', { p_payload: payload });
}

async function supabaseEliminarSupervisor(payload) {
    return supabaseRpc('deactivate_supervisor_profile_application', { p_payload: payload });
}

async function supabaseRegistrarDocumentoProyecto(payload) {
    return supabaseRpc('register_project_document_application', { p_payload: payload });
}

async function supabaseCargarWorkspaceUsuarios() {
    return supabaseRpc('get_user_access_workspace');
}

async function supabaseGuardarAccesoUsuario(payload) {
    return supabaseRpc('save_user_access_application', { p_payload: payload });
}

async function supabaseCrearUsuarioAdmin(payload) {
    DefenSecurity.rateLimit('edge:admin-create-user',5,600);
    DefenSecurity.validatePayload(payload,'admin-create-user');
    let token = await supabaseTokenVigente();
    const invocar = () => supabasePeticion('/functions/v1/admin-create-user', {
        method:'POST', headers:supabaseHeaders(token, true), body:JSON.stringify(payload)
    });
    try { return await invocar(); }
    catch (error) { if (error.status!==401) throw error; token=await supabaseRenovarSesion(); return invocar(); }
}

async function supabaseEliminarActaArmamento(codigo, motivo) {
    return supabaseRpc('delete_weapon_act_application', { p_act_code: codigo, p_reason: motivo });
}

async function supabaseCargarAuditoriaSistema(desde, hasta, modulo = 'TODOS', busqueda = '', limite = 1000) {
    return supabaseRpc('get_system_audit_workspace', {
        p_from: desde || null,
        p_to: hasta || null,
        p_module: modulo || 'TODOS',
        p_search: busqueda || null,
        p_limit: limite
    });
}

async function supabaseCargarSaludSistema() {
    return supabaseRpc('get_system_health_workspace');
}

async function supabaseSubirDocumentoProyecto(archivo, projectId, tipo) {
    ...
    const extension = extensiones[mime] || String(archivo?.name || '').split('.').pop().toLowerCase();
    if (!['pdf','xls','xlsx'].includes(extension)) throw new Error('El documento debe ser PDF, XLS o XLSX.');
    if (!archivo?.size || archivo.size > 10 * 1024 * 1024) throw new Error('El documento no puede superar 10 MB.');
    const tiposPorExtension = {
        pdf:'application/pdf',
        xls:'application/vnd.ms-excel',
        xlsx:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    };
    const contentType = extensiones[mime] ? mime : tiposPorExtension[extension];
    const categoria = String(tipo || 'OTRO').toLowerCase().replace(/[^a-z0-9]+/g, '-');
    const ruta = `${projectId}/${categoria}/${crypto.randomUUID()}.${extension}`;
    let token = await supabaseTokenVigente();
    const subir = async () => {
        const respuesta = await fetch(`${SUPABASE_URL}/storage/v1/object/project-documents/${supabaseRutaStorage(ruta)}`, {
            method:'POST', headers:{...supabaseHeaders(token),'Content-Type':contentType,'x-upsert':'false'}, body:archivo
        });
        ...
        const error = new Error(cuerpo?.message || cuerpo?.error || `No se pudo cargar el documento (HTTP ${respuesta.status}).`); error.status=respuesta.status; throw error;
    };
    try { return await subir(); }
    catch (error) { if (error.status!==401) throw error; token=await supabaseRenovarSesion(); return subir(); }
}

async function supabaseEliminarDocumentoProyecto(ruta) {
    if (!ruta) return;
    let token = await supabaseTokenVigente();
    const eliminar = () => supabasePeticion(`/storage/v1/object/project-documents/${supabaseRutaStorage(ruta)}`, {method:'DELETE',headers:supabaseHeaders(token)});
    try{return await eliminar();}catch(error){if(error.status!==401)throw error;token=await supabaseRenovarSesion();return eliminar();}
}

async function supabaseUrlFirmadaDocumentoProyecto(ruta, segundos = 300) {
    const respuesta = await supabaseRpcStorageFirmadaEnBucket('project-documents', ruta, segundos);
    const firmada = respuesta?.signedURL || respuesta?.signedUrl || '';
    if (!firmada) throw new Error('No se pudo generar el enlace temporal del documento.');
...
async function supabaseCargarIndicadoresHistorialArmamento() {
    const respuesta = await supabaseRpc('get_weapon_history_insights');
    if (!respuesta || respuesta.schema_version !== 1 || !Array.isArray(respuesta.weapons)) {
        throw new Error('Supabase devolvió indicadores históricos incompatibles.');
    }
    return respuesta;
}

async function supabaseCargarContextoRegularizaciones(codigos = []) {
    return supabaseRpc('get_weapon_regularization_context', { p_act_codes: codigos });
}
