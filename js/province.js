...
async function exportarPDF() {
    ...
    // ── 3. DETALLE POR PUESTO (todos los puestos de todos los proyectos) ──
    const puestosPorProyecto = puestosData[prov] || {};
    const todosLosPuestos = [];
    Object.entries(puestosPorProyecto).forEach(([nombreProy, lista]) => {
        lista.forEach(pu => todosLosPuestos.push({ ...pu, _proyecto: nombreProy }));
    });
    ...
    if (todosLosPuestos.length > 0) {
        ...
        doc.setFontSize(11); doc.setFont('helvetica','bold'); doc.setTextColor(...DARK);
        doc.text(`DETALLE POR PUESTO (${todosLosPuestos.length})`, 14, y); y += 5;
        ...
        doc.autoTable({
            ...
            head: [['Puesto','Proyecto','Guardia(s)','Tipo','Armado','Radio']],
            body: todosLosPuestos.map(pu => {
            ...
            styles: { fontSize:7, cellPadding:2, overflow:'linebreak', lineColor:[226,232,240], lineWidth:0.3 },
            alternateRowStyles: { fillColor:[248,250,252] },
            columnStyles: { 0:{cellWidth:34,fontStyle:'bold'}, 1:{cellWidth:44}, 4:{halign:'center'}, 5:{halign:'center'} }
        });
        y = doc.lastAutoTable.finalY + 10;
    }

    // ── 4. MAPA ESQUEMÁTICO CON TODOS LOS PUESTOS ACTIVOS ──
...
async function exportarPDFProyecto(nombreProyecto) {
    ...
    // ── 3. MAPA ESQUEMÁTICO SOLO DE ESTE PROYECTO ──
    if (puestos.length > 0) {
