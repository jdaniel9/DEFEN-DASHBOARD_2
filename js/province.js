...
function crearAcordeonProyecto(provincia, proyecto, idx) {
    ...
    header.innerHTML = `
        ...
        <div class="flex items-center gap-1.5 flex-shrink-0">
            ${renderBotonDocumentoProyecto(proyecto.urlDocumento, 'OC/CT', true)}
            ${renderBotonDocumentoProyecto(proyecto.urlKardex, 'KARDEX', true)}
            ${(typeof rolActual === 'function' && rolActual() === 'admin') ? `<button onclick="event.stopPropagation(); archivarProyectoActivo('${provincia}','${proyecto.nombre.replace(/'/g,"\\'")}')" class="text-[8px] font-black bg-amber-600 hover:bg-amber-700 text-white px-1.5 py-0.5 rounded-full" title="Archivar este proyecto (mover al histórico)">🗄️</button>` : ''}
    ...
    const guardiasHTML = guardias.length > 1
        ? `<div class="flex flex-col gap-0.5 mt-1">
            ${guardias.map((g) => {
                ...
                const contacto=(contactosPersonal||{})[String(g||'').toUpperCase().trim()]||{};
                return `
                <span class="text-[9px] text-slate-500 font-medium">
                    ...
                    <span class="block pl-4 text-[8px] text-slate-400">📞 ${contacto.telefono||'SIN TELÉFONO'} · INGRESO ${contacto.fechaIngreso||'—'}</span>
                </span>`;
            }).join('')}
           </div>`
        : (()=>{const g=guardias[0]||'',contacto=(contactosPersonal||{})[String(g).toUpperCase().trim()]||{};return `<p class="text-[9px] text-slate-400 font-medium">👮 ${g||'—'} · ${puesto.tipo}<span class="block pl-4 text-[8px]">📞 ${contacto.telefono||'SIN TELÉFONO'} · INGRESO ${contacto.fechaIngreso||'—'}</span></p>`;})();
