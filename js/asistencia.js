...
function asistenciaCodigoVisible(codigo) {
    return ({ '3':'DN', '4':'D', '5':'N', '6':'T' })[codigo] || codigo;
}
    ...
    modal.innerHTML = `
      <div class="as-shell">
        <header class="as-header">
          ...
          <div class="as-header-actions"><button id="as-btn-ingreso" onclick="abrirIngresoPersonalAsistencia()">➕ INGRESO</button><button id="as-btn-supervisor-interno" onclick="abrirIngresoSupervisorInternoAsistencia()">👔 SUPERVISOR INTERNO</button><button id="as-btn-cierre" onclick="abrirCierreMensualAsistencia()">📦 CIERRE MENSUAL</button><button id="as-btn-coberturas" onclick="abrirGestorCoberturasAsistencia()">👥 COBERTURAS</button><button onclick="recargarModuloAsistencia()">↻ ACTUALIZAR</button><button onclick="cerrarModuloAsistencia()">✕ CERRAR</button></div>
        </header>
        <div class="as-body">
          ...
          <section class="as-toolbar">
            ...
            <label>ESTADO<select id="as-filtro-estado" onchange="filtrarModuloAsistencia()"><option value="">TODOS</option><option>ACTIVO</option><option>APOYO</option><option>CAMBIO</option><option>VACANTE</option><option value="INACTIVO">PASIVO</option></select></label>
...
async function cargarWorkspaceAsistencia(periodId) {
    ...
    try {
        ...
        const [workspace,directorio] = await Promise.all([
            supabaseRpc('get_attendance_workspace', { p_period_id: periodId || null }),
            supabaseCargarDirectorioPersonal().catch(()=>null)
        ]);
        if (!workspace || workspace.schema_version !== 1) throw new Error('CONTRATO DE ASISTENCIA INCOMPATIBLE.');
        if(directorio){const personas=new Map((directorio.people||[]).map(p=>[String(p.id),p]));(workspace.assignments||[]).forEach(a=>{const p=personas.get(String(a.personnel_id||''));a.mobile_phone=p?.mobile_phone||'';});workspace.permissions=workspace.permissions||{};workspace.permissions.manage_contacts=Boolean(directorio.permissions?.manage_contacts);}
...
function prepararControlesAsistencia() {
    ...
    document.getElementById('as-leyenda').innerHTML = (w.codes || []).map(c => `<span class="${asistenciaCodigoClase(c.code)}"><b>${asistenciaEsc(asistenciaCodigoVisible(c.code))}</b> ${asistenciaEsc(c.label)}</span>`).join('');
    const puedeAdministrarPersonal = w.permissions?.manage_personnel !== false;
    const botonCierre = document.getElementById('as-btn-cierre');
    if (botonCierre) botonCierre.style.display = w.permissions?.close_month && w.period.status === 'OPEN' ? '' : 'none';
    ['as-btn-ingreso','as-btn-supervisor-interno'].forEach(id => {
        const boton = document.getElementById(id);
        if (boton) boton.style.display = w.permissions?.manage && puedeAdministrarPersonal && w.period.status === 'OPEN' ? '' : 'none';
    });
    const botonCoberturas = document.getElementById('as-btn-coberturas');
    if (botonCoberturas) botonCoberturas.style.display = w.permissions?.manage_coverages !== false ? '' : 'none';
}
...
function abrirSalidaPersonalAsistencia(assignmentId = '') {
    const w = asistenciaModulo.workspace;
    ...
    const seleccionada = assignmentId ? asignaciones.find(a => a.assignment_id === assignmentId) : asignaciones[0];
    if (!seleccionada) return alert('LA ASIGNACIÓN YA NO ESTÁ ACTIVA. ACTUALIZA LA TABLA E INTENTA NUEVAMENTE.');
    const opciones = asignaciones.map(a => `<option value="${asistenciaEsc(a.assignment_id)}" ${a.assignment_id === seleccionada.assignment_id ? 'selected' : ''}>${asistenciaEsc(a.full_name || 'SIN NOMBRE')} · ${asistenciaEsc(a.national_id || 'SIN CÉDULA')} · ${asistenciaEsc(a.project || 'SIN PROYECTO')} · ${asistenciaEsc(a.post || 'SIN PUESTO')}</option>`).join('');
    const editor = asistenciaEditorElemento();
    editor.innerHTML = `<div class="as-editor-card as-coverage-form"><h3>SALIDA DE PERSONAL</h3><p>REGISTRA EL ÚLTIMO DÍA TRABAJADO. <b>RENUNCIA</b> Y <b>OTROS</b> GENERAN UNA VACANTE; <b>TERMINACIÓN DE PROYECTO</b> CIERRA EL PUESTO SIN GENERARLA.</p><div class="as-form-grid"><label class="as-form-wide">PERSONAL<select id="as-salida-asignacion">${opciones}</select></label><label>ÚLTIMO DÍA TRABAJADO<input id="as-salida-fecha" type="date" min="${asistenciaEsc(w.period.month_start)}" max="${asistenciaEsc(asistenciaFinPeriodo())}" value="${asistenciaEsc(asistenciaHoyEcuador())}"></label><label class="as-form-wide">MOTIVO DE SALIDA<select id="as-salida-tipo" onchange="cambiarMotivoSalidaAsistencia()"><option value="">SELECCIONAR...</option><option value="RENUNCIA VOLUNTARIA">RENUNCIA VOLUNTARIA</option><option value="TERMINACION DE PROYECTO">TERMINACIÓN DE PROYECTO</option><option value="OTROS">OTROS</option></select></label><label id="as-salida-otro-wrap" class="as-form-wide" style="display:none;">DETALLE DE OTROS<textarea id="as-salida-otro" rows="3" maxlength="250" placeholder="ESPECIFICA EL MOTIVO"></textarea></label></div><div class="as-editor-actions"><button class="danger" onclick="guardarSalidaPersonalAsistencia()">CONFIRMAR SALIDA</button><button onclick="cerrarEditorAsistencia()">CANCELAR</button></div></div>`;
    editor.style.display = 'flex';
}

function cambiarMotivoSalidaAsistencia() {
    const esOtro = document.getElementById('as-salida-tipo')?.value === 'OTROS';
    const contenedor = document.getElementById('as-salida-otro-wrap');
    if (contenedor) contenedor.style.display = esOtro ? '' : 'none';
    if (!esOtro) {
        const detalle = document.getElementById('as-salida-otro');
...
async function guardarSalidaPersonalAsistencia() {
    ...
    const tipo = document.getElementById('as-salida-tipo')?.value || '';
    const detalleOtro = document.getElementById('as-salida-otro')?.value.trim() || '';
    if (!assignmentId || !fecha || !tipo) return alert('SELECCIONA EL PERSONAL, LA FECHA Y EL MOTIVO DE SALIDA.');
    if (tipo === 'OTROS' && detalleOtro.length < 3) return alert('ESPECIFICA EL MOTIVO EN LA OPCIÓN OTROS.');
    const motivo = tipo === 'OTROS' ? `OTROS: ${detalleOtro}` : tipo;
    const terminaProyecto = tipo === 'TERMINACION DE PROYECTO';
    const advertencia = terminaProyecto
        ? 'NO SE CREARÁ UNA VACANTE Y SE CERRARÁN SUS ASIGNACIONES EN ESTE PROYECTO.'
        : 'EL PUESTO SE CONVERTIRÁ EN VACANTE.';
    if (!confirm(`¿CONFIRMAS LA SALIDA? ${advertencia}`)) return;
    ...
    try {
        ...
        const vacantesCreadas = Number(respuesta?.vacancies_created ?? 0);
        alert(terminaProyecto
            ? 'SALIDA POR TERMINACIÓN DE PROYECTO REGISTRADA. NO SE GENERÓ VACANTE.'
            : `SALIDA REGISTRADA. ${vacantesCreadas} VACANTE(S) CREADA(S).`);
    } catch (error) {
...
function abrirIngresoSupervisorInternoAsistencia() {
    const w = asistenciaModulo.workspace;
    if (!w?.permissions?.manage || w.permissions?.manage_personnel === false || w.period.status !== 'OPEN') {
        return alert('SOLO ADMINISTRADOR U OPERACIONES PUEDEN REGISTRAR SUPERVISORES INTERNOS.');
    }
    const proyectos = (w.projects || []).map(p => `<option value="${asistenciaEsc(p.id)}">${asistenciaEsc(p.province)} · ${asistenciaEsc(p.name)}</option>`).join('');
    if (!proyectos) return alert('NO EXISTEN PROYECTOS ACTIVOS DISPONIBLES.');
    const editor = asistenciaEditorElemento();
    editor.innerHTML = `<div class="as-editor-card as-coverage-form"><h3>INGRESO DE SUPERVISOR INTERNO</h3><p>EL SUPERVISOR FORMARÁ PARTE DE LA ASISTENCIA Y DE LA DOTACIÓN DEL PROYECTO, PERO <b>NO CREARÁ UN PUESTO OPERATIVO</b>.</p><div class="as-form-grid"><label class="as-form-wide">PROYECTO<select id="as-supervisor-proyecto">${proyectos}</select></label><label class="as-form-wide">NOMBRES Y APELLIDOS<input id="as-supervisor-nombre" type="text" maxlength="150" placeholder="NOMBRE COMPLETO"></label><label>CÉDULA<input id="as-supervisor-cedula" type="text" inputmode="numeric" maxlength="10" placeholder="10 DÍGITOS"></label><label>TELÉFONO<input id="as-supervisor-telefono" type="text" inputmode="tel" maxlength="16" placeholder="OPCIONAL"></label><label>FECHA DE INGRESO<input id="as-supervisor-fecha" type="date" max="${asistenciaEsc(asistenciaFinPeriodo())}" value="${asistenciaEsc(asistenciaHoyEcuador())}"></label><label>JORNADA<select id="as-supervisor-horario"><option>8 HORAS</option><option>12 HORAS</option><option>24 HORAS</option><option>ADMINISTRATIVO</option></select></label><label class="as-form-wide">OBSERVACIÓN<textarea id="as-supervisor-nota" rows="3" maxlength="250" placeholder="DETALLE DEL INGRESO"></textarea></label></div><div class="as-editor-actions"><button class="primary" onclick="guardarIngresoSupervisorInternoAsistencia()">CONFIRMAR INGRESO</button><button onclick="cerrarEditorAsistencia()">CANCELAR</button></div></div>`;
    editor.style.display = 'flex';
    const cedula = document.getElementById('as-supervisor-cedula');
    cedula.addEventListener('input', () => { cedula.value = cedula.value.replace(/\D/g, '').slice(0, 10); });
}

async function guardarIngresoSupervisorInternoAsistencia() {
    if (asistenciaModulo.guardando) return;
    const projectId = document.getElementById('as-supervisor-proyecto')?.value;
    const nombre = document.getElementById('as-supervisor-nombre')?.value.trim().toUpperCase();
    const cedula = document.getElementById('as-supervisor-cedula')?.value.trim();
    const telefono = document.getElementById('as-supervisor-telefono')?.value.trim().replace(/[^+0-9]/g, '') || null;
    const fecha = document.getElementById('as-supervisor-fecha')?.value;
    const horario = document.getElementById('as-supervisor-horario')?.value;
    const nota = document.getElementById('as-supervisor-nota')?.value.trim() || null;
    if (!projectId || !nombre || nombre.length < 5 || !/^\d{10}$/.test(cedula || '') || !fecha || !horario) return alert('COMPLETA EL PROYECTO, NOMBRE, CÉDULA DE 10 DÍGITOS, FECHA Y JORNADA.');
    if (telefono && !/^\+?\d{7,15}$/.test(telefono)) return alert('EL TELÉFONO DEBE CONTENER ENTRE 7 Y 15 DÍGITOS.');
    if (!confirm('¿CONFIRMAS EL INGRESO DEL SUPERVISOR INTERNO? NO SE CREARÁ UN PUESTO NUEVO.')) return;
    asistenciaModulo.guardando = true;
    try {
        const respuesta = await supabaseRpc('register_attendance_internal_supervisor', { p_project_id: projectId, p_national_id: cedula, p_full_name: nombre, p_mobile_phone: telefono, p_start_date: fecha, p_schedule: horario, p_note: nota });
        cerrarEditorAsistencia();
        await refrescarCoberturasAsistencia();
        alert(`SUPERVISOR REGISTRADO COMO ${respuesta?.assignment_label || 'SUPERVISOR INTERNO'}.`);
    } catch (error) {
        alert(`NO SE PUDO REGISTRAR EL SUPERVISOR: ${error.message || error}`);
    } finally { asistenciaModulo.guardando = false; }
}
...
function estadoAsistenciaVisible(estado) {
    return estado === 'INACTIVO' ? 'PASIVO' : estado;
}
...
function renderModuloAsistencia() {
    ...
    document.getElementById('as-tbody').innerHTML = filas.map(a => {
        ...
        const puedeRegistrarSalida = puedeEditar && ['ACTIVO', 'CAMBIO'].includes(a.status) && !esApoyoCobertura;
        const coberturaHtml = cobertura
            ? `<div class="as-person-coverage">🛡 CUBIERTO POR <b>${asistenciaEsc(cobertura.replacement_name)}</b> · ${asistenciaEsc(cobertura.status)}</div>`
            : '';
        const accionHtml = `${puedeCrearCobertura
            ...
            ? `<button class="as-exit-row" onclick="abrirSalidaPersonalAsistencia('${asistenciaEsc(a.assignment_id)}')">↪ SALIDA</button>`
            : ''}`;
        const dayCells = Array.from({length:dias}, (_,i) => {
            ...
            const motivoBloqueo = !dentroVigencia
                ? `NO DISPONIBLE: VIGENCIA ${a.employment_start_date || 'SIN INICIO'} A ${a.employment_end_date || 'SIN FIN'}`
                : (!puedeEditar ? 'PERIODO CERRADO O SIN PERMISO' : '');
            return `<td class="as-day ${asistenciaCodigoClase(codigo)} ${esMesActual&&dia===hoy.getDate()?'as-today':''} ${editable?'as-editable':''}" title="${asistenciaEsc(motivoBloqueo)}" ${editable?`onclick="editarMarcacionAsistencia('${asistenciaEsc(a.assignment_id)}',${dia},'${asistenciaEsc(codigo)}')"`:''}>${codigo ? asistenciaEsc(asistenciaCodigoVisible(codigo)) : '·'}</td>`;
        }).join('');
        const contactoHtml=a.status!=='VACANTE'?`<span class="as-person-chip as-person-phone">📞 ${asistenciaEsc(a.mobile_phone||'SIN TELÉFONO')}${w.permissions?.manage_contacts&&a.personnel_id?` <button onclick="editarTelefonoAsistencia('${asistenciaEsc(a.personnel_id)}','${asistenciaEsc(a.mobile_phone||'')}')" title="Editar teléfono">✎</button>`:''}</span><span class="as-person-chip">↳ INGRESO ${asistenciaEsc(asistenciaFechaLocal(a.employment_start_date))}</span>${a.employment_end_date?`<span class="as-person-chip as-person-exit-date">↪ SALIDA ${asistenciaEsc(asistenciaFechaLocal(a.employment_end_date))}</span>`:''}`:'';
        return `<tr><td class="as-sticky as-col-person"><div class="as-person-card"><div class="as-person-head"><b title="${asistenciaEsc(nombre)}">${asistenciaEsc(nombre)}</b><span>CI ${asistenciaEsc(a.national_id || 'SIN CÉDULA')}</span></div><div class="as-person-location" title="${asistenciaEsc(ubicacion)}">📍 ${asistenciaEsc(ubicacion||'SIN UBICACIÓN')}</div><div class="as-person-meta"><span class="as-person-chip as-person-schedule">◷ ${asistenciaEsc(a.schedule || 'SIN HORARIO')}${a.mobile_coverage?' · MÓVIL':''}</span>${contactoHtml}</div>${coberturaHtml}<div class="as-person-actions">${accionHtml}</div></div></td><td class="as-sticky as-col-status"><span class="as-status as-status-${String(a.status||'').toLowerCase()}">${asistenciaEsc(estadoAsistenciaVisible(a.status))}</span></td>${dayCells}</tr>`;
...
async function editarTelefonoAsistencia(personnelId,telefonoActual=''){
    const telefono=prompt('Número de teléfono (7 a 15 dígitos):',telefonoActual||'');if(telefono===null)return;
    const limpio=telefono.trim().replace(/[^+0-9]/g,'');if(limpio&&!/^\+?[0-9]{7,15}$/.test(limpio))return alert('EL TELÉFONO DEBE CONTENER ENTRE 7 Y 15 DÍGITOS.');
    try{await supabaseActualizarContactoPersonal({request_id:crypto.randomUUID(),personnel_id:personnelId,mobile_phone:limpio});await recargarModuloAsistencia();if(typeof cargarDatos==='function')await cargarDatos();}
    catch(error){alert('NO SE PUDO ACTUALIZAR EL TELÉFONO: '+(error.message||error));}
}
