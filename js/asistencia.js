...
function renderModuloAsistencia() {
    ...
    document.getElementById('as-tbody').innerHTML = filas.map(a => {
        ...
        const dayCells = Array.from({length:dias}, (_,i) => {
            ...
            return `<td class="as-day ${asistenciaCodigoClase(codigo)} ${esMesActual&&dia===hoy.getDate()?'as-today':''} ${editable?'as-editable':''}" ${editable?`onclick="editarMarcacionAsistencia('${asistenciaEsc(a.assignment_id)}',${dia},'${asistenciaEsc(codigo)}')"`:''}>${codigo ? asistenciaEsc(asistenciaCodigoVisible(codigo)) : '·'}</td>`;
        }).join('');
