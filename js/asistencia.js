...
function renderModuloAsistencia() {
    ...
    document.getElementById('as-tbody').innerHTML = filas.map(a => {
        ...
        const contactoHtml=a.status!=='VACANTE'?`<small>📞 ${asistenciaEsc(a.mobile_phone||'SIN TELÉFONO')} ${w.permissions?.manage_contacts&&a.personnel_id?`<button onclick="editarTelefonoAsistencia('${asistenciaEsc(a.personnel_id)}','${asistenciaEsc(a.mobile_phone||'')}')" title="Editar teléfono" style="border:0;background:#e0f2fe;color:#0369a1;border-radius:5px;font-size:8px;font-weight:900;cursor:pointer">EDITAR</button>`:''}</small><small>INGRESO: ${asistenciaEsc(asistenciaFechaLocal(a.employment_start_date))} · SALIDA: ${asistenciaEsc(asistenciaFechaLocal(a.employment_end_date))}</small>`:'';
        return `<tr><td class="as-sticky as-col-person"><b>${asistenciaEsc(nombre)}</b><small>${asistenciaEsc(a.national_id || 'SIN CÉDULA')}</small><small>${asistenciaEsc(ubicacion)}</small><small>${asistenciaEsc(a.schedule || 'SIN HORARIO')}${a.mobile_coverage?' · COBERTURA MÓVIL':''}</small>${contactoHtml}${coberturaHtml}${accionHtml}</td><td class="as-sticky as-col-status"><span class="as-status as-status-${String(a.status||'').toLowerCase()}">${asistenciaEsc(estadoAsistenciaVisible(a.status))}</span></td>${dayCells}</tr>`;
