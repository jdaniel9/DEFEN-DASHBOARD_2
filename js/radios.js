...
async function abrirModalRadios() {
    ...
    try {
        if (backendUsaSupabase()) await cargarWorkspaceRadios();
        renderFiltrosRadios();
        renderTablaRadios();
        aplicarPermisosUI();
    } catch (error) {
...
function renderTablaRadios() {
    ...
    const puedeGestionar = typeof usuarioPuedeGestionarRadios === 'function'
        && usuarioPuedeGestionarRadios()
        && radioWorkspace?.permissions?.manage === true;
    if (!filtrados.length) {
    ...
    tbody.innerHTML = filtrados.map((r, i) => `
      <tr style="border-bottom:1px solid #f1f5f9;${i % 2 === 0 ? 'background:#f8fafc;' : ''}">
        ...
        <td style="padding:7px 8px;">${radioEsc(r.puesto || (r.proyecto ? 'RADIO ADICIONAL DEL PROYECTO' : '—'))}</td>
...
function abrirFormularioRadioNuevo() {
    if (!usuarioPuedeGestionarRadios() || radioWorkspace?.permissions?.manage !== true) return alert('SOLO ADMINISTRADOR O SISTEMAS PUEDE GESTIONAR RADIOS.');
...
function renderFormularioRadio() {
    ...
    document.getElementById('radio-editor-card').innerHTML = `
      ...
      <div style="padding:16px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px">
        ...
        <label id="radio-form-puesto-wrap" style="font-size:10px;font-weight:900;color:#475569">PUESTO (OPCIONAL)<select id="radio-form-puesto" style="width:100%;box-sizing:border-box;margin-top:4px;padding:9px;border:1px solid #cbd5e1;border-radius:8px"></select></label>
...
function actualizarPuestosFormularioRadio() {
    ...
    if (!puesto) return;
    puesto.innerHTML = radioOpciones(proyecto?.posts || [], puesto.value || radioEditorActual?.puestoId || '', 'SIN PUESTO · RADIO ADICIONAL DEL PROYECTO');
}
...
async function guardarFormularioRadio() {
    ...
    if (estado === 'ASIGNADO' && !proyecto) return alert('SELECCIONA EL PROYECTO. EL PUESTO ES OPCIONAL.');
...
function abrirHistorialRadios() {
    if (!usuarioPuedeVerHistorialRadios() || radioWorkspace?.permissions?.view_history !== true) return alert('NO TIENES PERMISO PARA VER ESTE HISTORIAL.');
