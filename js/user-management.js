...
// Perfiles, roles, bloqueo y alta administrativa segura de usuarios.
// ================================================================
...
function asegurarModalGestionUsuarios() {
    ...
    modal.innerHTML=`<div style="width:min(1200px,98vw);height:min(820px,95vh);background:#f1f5f9;border-radius:20px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 32px 100px rgba(0,0,0,.55)">
      ...
      <div style="padding:10px 14px;background:#ede9fe;border-bottom:1px solid #ddd6fe;color:#4c1d95;font-size:10px;display:flex;align-items:center;gap:10px"><div style="flex:1"><b>Administración de cuentas:</b> solo el Administrador puede crear usuarios y asignar su perfil. Las contraseñas se envían de forma segura y nunca se muestran ni almacenan en este panel.</div><button onclick="abrirFormularioNuevoUsuario()" style="border:0;border-radius:8px;background:#7c3aed;color:white;padding:8px 11px;font-weight:900;cursor:pointer">＋ CREAR USUARIO</button></div>
...
function abrirFormularioNuevoUsuario() {
    const c=document.getElementById('gu-contenido');
    const roles=(gestionUsuarios.workspace?.roles||[]).filter(r=>r.code!=='sin_asignar').map(r=>`<option value="${guEsc(r.code)}">${guEsc(r.label)}</option>`).join('');
    c.innerHTML=`<section style="background:white;border:1px solid #cbd5e1;border-top:5px solid #7c3aed;border-radius:14px;padding:16px"><h2 style="margin:0 0 13px;font-size:17px">CREAR NUEVO USUARIO</h2><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px"><label>NOMBRE COMPLETO<input id="gu-nuevo-nombre" maxlength="150" style="width:100%;box-sizing:border-box;padding:9px;margin-top:4px"></label><label>USUARIO<input id="gu-nuevo-usuario" maxlength="40" placeholder="ej. jperez" style="width:100%;box-sizing:border-box;padding:9px;margin-top:4px"></label><label>PERFIL<select id="gu-nuevo-rol" style="width:100%;padding:9px;margin-top:4px">${roles}</select></label><label>CONTRASEÑA TEMPORAL<input id="gu-nuevo-clave" type="password" minlength="12" maxlength="128" autocomplete="new-password" style="width:100%;box-sizing:border-box;padding:9px;margin-top:4px"></label><label>REPETIR CONTRASEÑA<input id="gu-nuevo-clave2" type="password" minlength="12" maxlength="128" autocomplete="new-password" style="width:100%;box-sizing:border-box;padding:9px;margin-top:4px"></label></div><p style="font-size:9px;color:#64748b">Mínimo 12 caracteres con mayúscula, minúscula, número y símbolo. Entrégala directamente al usuario; no quedará visible después.</p><div style="display:flex;gap:8px"><button onclick="crearNuevoUsuarioAdmin()" style="flex:2;border:0;border-radius:9px;background:#7c3aed;color:white;padding:10px;font-weight:900">CREAR CUENTA</button><button onclick="renderDetalleGestionUsuario()" style="flex:1;border:1px solid #cbd5e1;border-radius:9px;background:white">CANCELAR</button></div></section>`;
}

async function crearNuevoUsuarioAdmin(){
    if(gestionUsuarios.guardando)return;
    const username=document.getElementById('gu-nuevo-usuario').value.trim().toLowerCase(),full_name=document.getElementById('gu-nuevo-nombre').value.trim(),role_code=document.getElementById('gu-nuevo-rol').value,password=document.getElementById('gu-nuevo-clave').value,password2=document.getElementById('gu-nuevo-clave2').value;
    if(!/^[a-z0-9._-]{3,40}$/.test(username))return alert('EL USUARIO DEBE TENER ENTRE 3 Y 40 CARACTERES: LETRAS, NÚMEROS, PUNTO, GUION O GUION BAJO.');
    if(full_name.length<5)return alert('ESCRIBE EL NOMBRE COMPLETO.');
    if(password.length<12||password.length>128||!/[a-z]/.test(password)||!/[A-Z]/.test(password)||!/[0-9]/.test(password)||!/[^A-Za-z0-9]/.test(password))return alert('LA CONTRASEÑA DEBE TENER 12 A 128 CARACTERES, MAYÚSCULA, MINÚSCULA, NÚMERO Y SÍMBOLO.');
    if(password!==password2)return alert('LAS CONTRASEÑAS NO COINCIDEN.');
    gestionUsuarios.guardando=true;
    try{const r=await supabaseCrearUsuarioAdmin({request_id:guRequestId(),username,full_name,role_code,password});await cargarGestionUsuarios(r.user_id||'');alert('USUARIO CREADO Y HABILITADO CORRECTAMENTE.');}
    catch(error){alert('NO SE PUDO CREAR EL USUARIO: '+(error.message||error));}
