...
function custodiosArmamentoUbicacion(provincia,proyecto,puesto=''){
    ...
    return armas.filter(a=>['activo','en campo'].includes(normalizarTexto(a.estado))&&normalizarTexto(a.provincia)===normalizarTexto(provincia)&&normalizarTexto(a.proyecto)===normalizarTexto(proyecto)&&(!puesto||normalizarTexto(a.puesto)===normalizarTexto(puesto))&&a.responsableNombre).map(a=>({nombre:a.responsableNombre,cedula:a.responsableCedula||'',ubicacion:a.ubicacion||''})).filter(c=>{const k=normalizarTexto(c.cedula||c.nombre);if(vistos.has(k))return false;vistos.add(k);return true;});
}
