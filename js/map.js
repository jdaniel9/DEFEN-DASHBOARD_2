...
function custodiosArmamentoUbicacion(provincia,proyecto,puesto=''){
    const armas=Array.isArray(window.armamentoDetalle)?window.armamentoDetalle:(typeof armamentoDetalle!=='undefined'&&Array.isArray(armamentoDetalle)?armamentoDetalle:[]),vistos=new Set();
    return armas.filter(a=>['activo','en campo'].includes(normalizarTexto(a.estado))&&normalizarTexto(a.provincia)===normalizarTexto(provincia)&&normalizarTexto(a.proyecto)===normalizarTexto(proyecto)&&(!puesto||normalizarTexto(a.puesto)===normalizarTexto(puesto))).map(a=>{const ubicacion=String(a.ubicacion||'').trim(),generica=/^(custodio|pdg|dotaci[oó]n).*\(\d+\)$/i.test(ubicacion),cedula=a.responsableCedula||ubicacion.match(/\d{10}/)?.[0]||'',nombre=a.responsableNombre||(!generica?ubicacion.replace(/\s*[·-]?\s*(CI\.?\s*)?\d{10}.*$/i,'').trim():'');return{nombre,cedula,ubicacion};}).filter(c=>c.nombre&&!/^\d+$/.test(c.nombre)).filter(c=>{const k=normalizarTexto(c.cedula||c.nombre);if(vistos.has(k))return false;vistos.add(k);return true;});
}

function duracionProyectoMeses(inicio,fin){
    const leer=v=>{const m=String(v||'').match(/^(\d{4})-(\d{2})-(\d{2})/);return m?Date.UTC(Number(m[1]),Number(m[2])-1,Number(m[3])):null;},desde=leer(inicio),hasta=leer(fin);
    if(desde===null||hasta===null||hasta<desde)return '—';
    const dias=Math.floor((hasta-desde)/86400000)+1,meses=Math.max(1,Math.round(dias/30.4375));
    ...
    const proyectosHTML = tieneProy
        ? listaProy.map(p => {
            ...
            const supsP = p.supervisores && p.supervisores.length > 0
                ? `<div class="flex flex-wrap gap-1 mt-1">${p.supervisores.map(s => {const c=(contactosPersonal||{})[String(s||'').toUpperCase().trim()]||{};return `<span class="text-[9px] font-bold text-blue-700 bg-blue-50 border border-blue-100 px-1.5 py-0.5 rounded-full">👤 ${s} · 📞 ${c.telefono||'SIN TELÉFONO'}</span>`;}).join('')}</div>`
                : '';
            ...
            const puestosListHTML = puestosProy.length > 0
                ? puestosProy.map(pu => {
                    ...
                    const custodiosPuesto=custodiosArmamentoUbicacion(nombre,p.nombre,pu.nombre);
                    const agentesHTML = gs.length > 0
                        ? gs.map(g => {const c=(contactosPersonal||{})[String(g||'').toUpperCase().trim()]||{};return `<span class="block text-[9px] text-slate-500 font-semibold">👤 ${g}<small class="block pl-4 text-[8px] text-slate-400">📞 ${c.telefono||'SIN TELÉFONO'} · INGRESO ${c.fechaIngreso||'—'}</small></span>`;}).join('')
                        : custodiosPuesto.length?custodiosPuesto.map(c=>`<span class="block text-[9px] text-slate-600 font-semibold">👤 ${c.nombre}<small class="block pl-4 text-[8px] text-slate-400">CUSTODIO${c.cedula?' · CI '+c.cedula:''}</small></span>`).join(''):`<span class="block text-[9px] text-slate-400 italic">Sin agentes asignados</span>`;
            ...
            const custodiosProyecto=custodiosArmamentoUbicacion(nombre,p.nombre),guardiasProyecto=Number(p.guardias)||0,cantidadPersonas=guardiasProyecto||custodiosProyecto.length,etiquetaPersonas=guardiasProyecto?'guardia(s)':'custodio(s)';
            return `
            <div class="project-row bg-white flex flex-col gap-2 ${p.vacantes > 0 ? 'ring-2 ring-red-300' : ''}">
                <div class="flex items-start justify-between gap-2 cursor-pointer" onclick="document.getElementById('${idSafe}').classList.toggle('hidden'); this.querySelector('.chev-puestos').classList.toggle('rotate-180')">
                    ...
                    <div class="flex items-center gap-1.5 flex-shrink-0">
                        ${renderBotonDocumentoProyecto(p.urlDocumento, 'OC/CT', false)}
                        ${renderBotonDocumentoProyecto(p.urlKardex, 'KARDEX', false)}
                ...
                <div class="flex flex-wrap gap-3 text-[11px] text-slate-500 font-semibold">
                    <span>👮 ${cantidadPersonas} ${etiquetaPersonas}</span>
                    <span>🔫 ${p.armas} arma(s)</span>
                    <span>🏢 ${p.puestos ?? '—'} puesto(s)</span>
                    <span>📅 Ingresa: ${formatFecha(p.inicio)}</span>
                    <span>📅 Finaliza: ${formatFecha(p.fin)}</span>
                    <span>⏱️ Duración: ${duracionProyectoMeses(p.inicio,p.fin)}</span>
                </div>
                ${supsP}
